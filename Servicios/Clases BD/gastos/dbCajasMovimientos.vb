Public Class dbCajasMovimientos
    ...
    Public Sub LlenaDatos()
        ...
        Comm.CommandText = "select * from tblcajasmovimientos where idmovimiento=" + ID.ToString
    ...
    Public Sub Guardar(ByVal pIdCliente As Integer, ByVal pFecha As String, ByVal pFolio As Integer, ByVal pDesglosar As Byte, ByVal pIva As Double, ByVal pidSucursal As Integer, ByVal pSerie As String, ByVal pidCaja As Integer, ByVal pIdVendedor As Integer, ByVal pTipo As Byte)
        ...
        IdCaja = pidCaja
        Tipo = pTipo
        Comm.CommandText = "insert into tblcajasmovimientos(fecha,folio,total,hora,estado,iva,totalapagar,desglosar,idsucursal,serie,idcaja,usado,idvendedor,comentario,tipo,idUsuarioAlta,fechaAlta,horaAlta,idUsuarioCambio,fechaCambio,horaCambio) values('" + Fecha + "'," + Folio.ToString + ",0,'" + Format(TimeOfDay, "HH:mm:ss") + "'," + CStr(Estados.SinGuardar) + "," + Iva.ToString + ",0," + Desglosar.ToString + "," + IdSucursal.ToString + ",'" + Replace(Trim(Serie), "'", "''") + "'," + IdCaja.ToString + ",0," + IdVendedor.ToString + ",''," + Tipo.ToString + "," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "'," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "');"
        'Comm.ExecuteNonQuery()
        Comm.CommandText += "select max(idmovimiento) from tblcajasmovimientos;"
        ID = Comm.ExecuteScalar
    End Sub
    Public Sub Modificar(ByVal pID As Integer, ByVal pFecha As String, ByVal pFolio As Integer, ByVal pDesglosar As Byte, ByVal pIva As Double, ByVal pEstado As Byte, ByVal pTotal As Double, ByVal pTotalaPagar As Double, ByVal pIdCliente As Integer, ByVal pSerie As String, ByVal pIdVendedor As Integer, ByVal pComentario As String, ByVal pTipo As Byte, ByVal pIdCaja As Integer)
        ID = pID
        Fecha = pFecha
        Folio = pFolio
        Desglosar = pDesglosar
        ...
        Estado = pEstado
        IdCaja = pIdCaja
        Comm.CommandText = "update tblcajasmovimientos set fecha='" + Fecha + "',folio=" + Folio.ToString + ",desglosar=" + Desglosar.ToString + ",iva=" + Iva.ToString + ",estado=" + Estado.ToString + ",total=" + Total.ToString + ",totalapagar=" + TotalaPagar.ToString + ",serie='" + Replace(Trim(Serie), "'", "''") + "',idvendedor=" + IdVendedor.ToString + ",comentario='" + Replace(Comentario, "'", "''") + "',tipo=" + Tipo.ToString + ",idcaja=" + IdCaja.ToString + ", idUsuarioCambio=" + GlobalIdUsuario.ToString() + ", fechaCambio='" + DateTime.Now.ToString("yyyy/MM/dd") + "', horaCambio='" + TimeOfDay.ToString("HH.mm:ss") + "' where idmovimiento=" + ID.ToString
        Comm.ExecuteNonQuery()
    End Sub
    Public Sub ActualizaComentario(ByVal pidnota As Integer, ByVal pTexto As String)
        Comm.CommandText = "update tblcajasmovimientos set comentario='" + Replace(pTexto, "'", "''") + "' where idmovimiento=" + pidnota.ToString
        Comm.ExecuteNonQuery()
    End Sub
    Public Sub Eliminar(ByVal pID As Integer)
        Comm.CommandText = "delete from tblcajasmovimientos where idmovimiento=" + pID.ToString
        Comm.ExecuteNonQuery()
    End Sub
    Public Function Consulta(ByVal pFecha As String, ByVal pFecha2 As String, ByVal pEstado As Byte, ByVal pFolio As String, ByVal pTipo As Byte, ByVal pidSucursal As Integer, ByVal pidCaja As Integer) As DataView
        Dim DS As New DataSet
        Comm.CommandText = "select tblcajasmovimientos.idmovimiento,tblcajasmovimientos.fecha,concat(tblcajasmovimientos.serie,convert(tblcajasmovimientos.folio using utf8)),round(tblcajasmovimientos.totalapagar) as totalapagar,if(tblcajasmovimientos.tipo=0,'INGRESO','RETIRO'),tblcajas.nombre as caja,case tblcajasmovimientos.estado when 2 then 'Pendiente' when 3 then 'Guardado' when 4 then 'Cancelada' end  as sestado from tblcajasmovimientos inner join tblcajas on tblcajasmovimientos.idcaja=tblcajas.idcaja where tblcajasmovimientos.fecha>='" + pFecha + "' and tblcajasmovimientos.fecha<='" + pFecha2 + "'"
        ...
        If pEstado <> Estados.Inicio Then
            Comm.CommandText += " and tblcajasmovimientos.estado=" + pEstado.ToString
        Else
            Comm.CommandText += " and tblcajasmovimientos.estado<>1"
        End If
        If pTipo > 0 Then
            Comm.CommandText += " and tblcajasmovimientos.tipo=" + CStr(pTipo - 1)
        End If
        ...
        End If
        If pFolio <> "" Then
            Comm.CommandText += " and concat(tblcajasmovimientos.serie,convert(tblcajasmovimientos.folio using utf8)) like '%" + Replace(pFolio, "'", "''") + "%'"
        End If

        Comm.CommandText += " order by tblcajasmovimientos.fecha desc,tblcajasmovimientos.serie,tblcajasmovimientos.folio desc"
        Dim DA As New MySql.Data.MySqlClient.MySqlDataAdapter(Comm)
        DA.Fill(DS, "tblcajasmovimientos")
        Return DS.Tables("tblcajasmovimientos").DefaultView
    End Function
    ...
    Public Function DaTotal(ByVal pidCotizacion As Integer, ByVal pidMoneda As Integer) As Double
        ...
        '+++++++++++++++++++++++++++++++++++++++Total por Articulos ++++++++++++++++++++++++++++++++++++
        Comm.CommandText = "select iddetalle from tblcajasmovimientosdetalles where idmovimiento=" + pidCotizacion.ToString
        DReader = Comm.ExecuteReader
        While DReader.Read
            IDs.Add(DReader("iddetalle"))
        End While
        DReader.Close()
        While Cont <= IDs.Count
            Comm.CommandText = "select precio from tblcajasmovimientosdetalles where iddetalle=" + IDs.Item(Cont).ToString
            Precio = Comm.ExecuteScalar
            Comm.CommandText = "select iva from tblcajasmovimientosdetalles where iddetalle=" + IDs.Item(Cont).ToString
            iIva = Comm.ExecuteScalar
            Comm.CommandText = "select idmoneda from tblcajasmovimientosdetalles where iddetalle=" + IDs.Item(Cont).ToString
            IdMonedaC = Comm.ExecuteScalar
            If pidMoneda = 2 Then
    ...
    Public Function DaNuevoFolio(ByVal pSerie As String, ByVal pidSucursal As Integer) As Integer
        Comm.CommandText = "select ifnull((select max(folio) from tblcajasmovimientos where serie='" + Replace(Trim(pSerie), "'", "''") + "' and (estado=3 or estado=4)),0)"
    ...
    Public Function ChecaFolioRepetido(ByVal pFolio As Integer, ByVal pSerie As String) As Boolean
        ...
        Comm.CommandText = "select count(folio) from tblcajasmovimientos where folio=" + pFolio.ToString + " and serie='" + Replace(Trim(pSerie), "'", "''") + "' and estado<>1 and estado<>2"
    ...
    Public Function DaIvas(ByVal pIdPedido As Integer) As MySql.Data.MySqlClient.MySqlDataReader
        Comm.CommandText = "select iva,precio,idmoneda from tblcajasmovimientosdetalles where idmovimiento=" + pIdPedido.ToString
    ...
    Public Function Reporte(ByVal pFecha1 As String, ByVal pFecha2 As String, ByVal pIdSucursal As Integer, ByVal pIdCliente As Integer, ByVal pMostrarEnPesos As Byte, ByVal pSoloCanceladas As Boolean, ByVal pIdVendedor As Integer, ByVal pSerie As String, ByVal pIdCaja As Integer, ByVal pTipo As Byte, ByVal pMostrarOC As Byte, ByVal pSerieOc As String) As DataView
        ...
        'Else
        'Comm.CommandText = "select v.idpedido idventa,v.fecha,v.folio,v.serie,v.estado,v.total as total,v.totalapagar as totalapagar,v.fecha,0 as tipodecambio,2 as idmoneda,v.usado " + _
        '"from tblcajasmovimientos v where v.fecha>='" + pFecha1 + "' and v.fecha<='" + pFecha2 + "'"
        'End If
        Comm.CommandText = "delete from tblrepcajasmovimientos"
        Comm.ExecuteNonQuery()
        Comm.CommandText = "insert into tblrepcajasmovimientos(idmovimiento,fecha,folio,serie,estado,descripcion,precio,tipo,idcaja,nombre) " + _
        "select v.idmovimiento,v.fecha,v.folio,v.serie,v.estado,vd.descripcion,vd.precio,v.tipo,v.idcaja,c.nombre from tblcajasmovimientos as v inner join tblcajasmovimientosdetalles as vd on v.idmovimiento=vd.idmovimiento inner join tblcajas as c on v.idcaja=c.idcaja where v.fecha>='" + pFecha1 + "' and v.fecha<='" + pFecha2 + "' "
        ...
        If pTipo <> 0 Then
            'pTipo = pTipo - 1
            Comm.CommandText += " and v.tipo=" + CStr(pTipo - 1)
        End If
        ...
        If pTipo <> 2 Then
            Comm.CommandText = "insert into tblrepcajasmovimientos(idmovimiento,fecha,folio,serie,estado,descripcion,precio,tipo,idcaja,nombre) " + _
            "select vr.idremision,vr.fecha,vr.folio,vr.serie,vr.estado,'Venta',vr.totalapagar,2,vr.idcaja,c.nombre from tblventasremisiones vr inner join tblformasdepagoremisiones fpr on vr.idforma=fpr.idforma inner join tblcajas c on vr.idcaja=c.idcaja where fpr.tipo=1 and vr.fecha>='" + pFecha1 + "' and vr.fecha<='" + pFecha2 + "' "
            If pIdSucursal > 0 Then Comm.CommandText += " and vr.idsucursal=" + pIdSucursal.ToString
            'If pIdCliente > 0 Then Comm.CommandText += " and v.idcliente=" + pIdCliente.ToString
            If pIdVendedor > 0 Then Comm.CommandText += " and vr.idvendedor=" + pIdVendedor.ToString
            If pSoloCanceladas Then
            ...
            If pMostrarOC = 1 Then
                ...
                'Comm.CommandText += " and vr.serie<>'" + Replace(pSerieOc, "'", "''") + "'"
            End If
            Comm.ExecuteNonQuery()
        End If
        Comm.CommandText = "select idmovimiento,fecha,folio,serie,estado,descripcion,precio,tipo,idcaja,nombre from tblrepcajasmovimientos order by tipo,fecha,serie,folio"
        Dim DA As New MySql.Data.MySqlClient.MySqlDataAdapter(Comm)
        DA.Fill(DS, "tblcajasmovimientos")
        'DS.WriteXmlSchema("tblcajasmovimientos.xml")
        Return DS.Tables("tblcajasmovimientos").DefaultView
    End Function
    Public Function BuscaMovimiento(ByVal pFolio As String) As Integer
        Comm.CommandText = "select ifnull((select idmovimiento from tblcajasmovimientos where concat(tblcajasmovimientos.serie,convert(tblcajasmovimientos.folio using utf8))='" + Replace(pFolio, "'", "''") + "' and estado=3 limit 1),0)"
        Return Comm.ExecuteScalar
    End Function
    Public Function DaId(ByVal pFolio As String) As Integer
        Comm.CommandText = "select ifnull((select idmovimiento from tblcajasmovimientos where tblcajasmovimientos.estado=3 and usado=0 and concat(tblcajasmovimientos.serie,convert(tblcajasmovimientos.folio using utf8))='" + Replace(pFolio, "'", "''") + "'),0)"
        Return Comm.ExecuteScalar
    End Function
    Public Sub Usar(ByVal pid As Integer)
        Comm.CommandText = "update tblcajasmovimientos set usado=1 where idmovimiento=" + pid.ToString
        Comm.ExecuteNonQuery()
    End Sub
    Public Function DaUltimaFecha() As String
        Dim F As String
        Comm.CommandText = "select ifnull((select max(fecha) from tblcajasmovimientos),'-')"

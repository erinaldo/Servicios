Public Class dbContabilidadMascarasDetalles
    ...
    Public Function Consulta(ByVal pIdMascara As Integer, pN1 As Integer, pN2 As Integer, pN3 As Integer, pN4 As Integer, pN5 As Integer) As DataView
        ...
        Comm.CommandText = "select iddetalle,cv.descripcion as Variable,case cv.modulo when 0 then 'Ventas' when 1 then 'Compras' when 2 then 'Ventas Dev.' when 3 then 'Compras Dev.' when 4 then 'Depósitos' when 5 then 'Pagos' when 6 then 'N. CR. Ventas' when 7 then 'N. CR. Compras' when 8 then 'NC Ventas' when 9 then 'NC Compras' when 10 then 'Nómina' when 11 then 'Mov. Inventario' when 12 then 'Documentos Prov.' end as Modulo,if(cm.idcuenta>0,(select concat(LPAD(c.cuenta," + pN1.ToString + ",'0'),' ',if(c.n2='','',LPAD(c.n2," + pN2.ToString + ",'0')),' ',if(c.n3='','',LPAD(c.n3," + pN3.ToString + ",'0')),' ',if(c.n4='','',LPAD(c.n4," + pN4.ToString + ",'0')),' ',if(c.n5='','',LPAD(c.n5," + pN5.ToString + ",'0'))) from tblccontables c where cm.idcuenta=c.idccontable)," +
            "case cm.idcuenta when -1 then 'COMODÍN PERCEPCIONES' when -2 then 'COMODÍN DEDUCCIONES' when -3 then 'COMODÍN CUENTAS BANCOS' when -4 then 'COMODÍN CLIENTES' when -5 then 'COMODÍN PROVEEDORES' WHEN -6 THEN 'COMODÍN CLIENTES' when -7 then 'COMODÍN PROVEEDORES' when -8 then 'COMODÍN TRABAJADORES' when -9 then 'COMODÍN CUENTAS BANCOS 2' when -10 then 'COMODÍN CUENTAS BANCOS 3' when -11 then 'COMODÍN CUENTAS BANCOS 4' " +
            "when -12 then 'COMODÍN CLIENTES 2' when -13 then 'COMODÍN CLIENTES 3' when -14 then 'COMODÍN CLIENTES 4' when -15 then 'COMODÍN PROVEEDORES 2' when -16 then 'COMODÍN PROVEEDORES 3' when -17 then 'COMODÍN PROVEEDORES 4' WHEN -18 THEN 'COMODÍN CLIENTES 2' WHEN -19 THEN 'COMODÍN CLIENTES 3' WHEN -20 THEN 'COMODÍN CLIENTES 4' when -21 then 'COMODÍN PROVEEDORES 2' when -22 then 'COMODÍN PROVEEDORES 3' when -23 then 'COMODÍN PROVEEDORES 4' when -24 then 'COMODÍN TRABAJADORES 2' when -25 then 'COMODÍN TRABAJADORES 3' when -26 then 'COMODÍN TRABAJADORES 4' " +
            "when -27 then 'COMODÍN ALMACENES' when -28 then 'COMODÍN ALMACENES 2' when -29 then 'COMODÍN ALMACENES 3' when -30 then 'COMODÍN ALMACENES 4' " +
            "end) as Cuenta," +
            "if(cm.idcuenta>0,(select descripcion from tblccontables where cm.idcuenta=tblccontables.idccontable),'COMODIN') as desccuenta,if(cm.cargo=0,'',if(cm.negativo=0,'1','-1')) as Cargo,if(cm.abono=0,'',if(cm.negativo=0,'1','-1')) as Abono," +
        ...
        'DS.WriteXmlSchema("mascaradetalles.xml")
    ...
    Public Function ModulosUsados(ByVal pidMascara As Integer) As Collection
        ...
        Return modulos
    End Function
    Public Function ComodinesUsados(ByVal pidMascara As Integer) As Collection
        Dim DR As MySql.Data.MySqlClient.MySqlDataReader
        Dim modulos As New Collection
        Comm.CommandText = "select distinct idcuenta from tblcontabilidadmascarasd where idmascara=" + pidMascara.ToString + " and idcuenta<0"
        DR = Comm.ExecuteReader
        While DR.Read
            modulos.Add(DR("idcuenta"))
        End While
        DR.Close()
        Return modulos
    End Function

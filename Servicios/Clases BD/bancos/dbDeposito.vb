...
Public Class dbDeposito
    ...
    Public Sub Modificar(ByVal pFecha As String, ByVal pReferencia As String, ByVal pBanco As String, ByVal pCantidad As String, ByVal pID As Integer, ByVal pBanco2 As String, ByVal pComentario As String, pFechaConta As String)
        'Modificar
        Comm.CommandText = "update tbldepostito set Fecha='" + Replace(pFecha, "'", "''") + "', Referencia='" + Replace(pReferencia, "'", "''") + "', Banco=" + pBanco + ", Cantidad=" + pCantidad + " ,Banco2=" + pBanco2 + ", comentario='" + Replace(pComentario, "'", "''") + "', idUsuarioCambio=" + GlobalIdUsuario.ToString() + ", fechaCambio='" + DateTime.Now.ToString("yyyy/MM/dd") + "', horaCambio='" + TimeOfDay.ToString("HH:mm:ss") + "',fechaconta='" + pFechaConta + "' where idDeposito=" + pID.ToString()
        Comm.ExecuteNonQuery()
    End Sub
    Public Function filtroTodos(pFecha1 As String, pFecha2 As String, pReferenca As String, pIdCuenta As Integer, pFolio As String) As DataView
        ...
        Comm.CommandText = "select iddeposito,fecha,referencia,cantidad,tblcuentas.numero,(select nombre from tblbancoscatalogo where idbanco=tblcuentas.banco) from tbldepostito inner join tblcuentas on tbldepostito.banco2=tblcuentas.idcuenta where tbldepostito.fecha>='" + pFecha1 + "' and tbldepostito.fecha<='" + pFecha2 + "'"
        ...
        End If
        If pFolio <> "" Then
            Comm.CommandText += " and convert(iddeposito using utf8) like '%" + Replace(pFolio, "'", "''") + "%'"
        End If
    ...
    Public Function TieneDepositos(pidVenta As Integer) As Boolean
        ...
        End If
    End Function
    Public Function DatotalDepositosConta(pFecha As String, pComent As String) As Double
        Comm.CommandText = "select ifnull((select sum(round(cantidad,2)) from tbldepostito where fechaconta='" + pFecha + "' and (select count(vd.iddeposito) from tblventasdepositos vd where vd.iddeposito=tbldepostito.iddeposito)=0 and (select count(vd.iddeposito) from tblventaspagos vd where vd.iddeposito=tbldepostito.iddeposito)=0"
        If pComent <> "" Then
            Comm.CommandText += " and comentario like '%" + pComent.Replace("'", "''") + "%'"
        End If
        Comm.CommandText += "),0)"

...
#Region "atributos"
    ...
    Dim INSERT As String = "insert into tblsemillasboletas(" +
                                ...
                                "porcentajeGranoD," +
                                "liquidada," +
                                "importe,iddetalle,precio,tara,brutosinanalizar,horasalida,idCliente,serie,idUsuarioAlta,fechaAlta,horaAlta,idUsuarioCambio,fechaCambio,horaCambio) " +
                                "values"
...
#Region "métodos"
    Public Sub agregar(ByRef boleta As dbSemillasBoleta)
        ...
        End If
        comm.CommandText += ",'" + boleta.serie + "'," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "'," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "');"
        Try
    ...
    Public Sub actualiza(ByRef boleta As dbSemillasBoleta)
        ...
        End If
        comm.CommandText += ", idUsuarioCambio=" + GlobalIdUsuario.ToString() + ", fechaCambio='" + DateTime.Now.ToString("yyyy/MM/dd") + "', horaCambio='" + TimeOfDay.ToString("HH:mm:ss") + "' where id=" + boleta.id.ToString() + ";"
        Try
            comm.ExecuteNonQuery()
            PopUp("Guardada", 10)
        Catch ex As Exception
    ...
    Public Function buscar(ByRef boleta As dbSemillasBoleta) As dbSemillasBoleta
        ...
        Else
            While dr.Read()
                ...
                If IsDBNull(dr("proveedor")) = False Then
                    idProveedor = dr.GetInt32("proveedor")
                End If
                b.chofer = dr.GetString("chofer")
                idInventario = dr.GetInt32("producto")
                b.peso = dr.GetDouble("peso")
                b.humedad = dr.GetDouble("humedad")
                b.impurezas = dr.GetDouble("impurezas")
                b.granoDanado = dr.GetDouble("granoD")
        ...
        End If
        dr.Close()
        b.producto = New dbInventario(idInventario, MySqlcon)
        If idProveedor > 0 Then
            b.productor = New dbproveedores(idProveedor, MySqlcon)
        End If
        Return b
    End Function
...
"where b.id=" + idboleta.ToString() + "),0) as id"
    ...
    Public Function buscaProductoProductor(ByVal claveProductor As String, ByVal claveCliente As String, ByVal claveProducto As String, pFecha As String, pFecha2 As String, pFolio As String, tipo As String) As DataView
       ...
       " inner join tblinventario as prod on prod.idinventario=b.producto inner join tblclientes as cli on cli.idcliente=b.idCliente where b.fecha>='" + pFecha + "' and b.fecha<='" + pFecha2 + "'"
        ...
        End If
        comm.CommandText += " order by b.fecha desc,b.serie,b.folio desc"
        Dim dc As New MySql.Data.MySqlClient.MySqlDataAdapter(comm)
        Dim ds As New DataSet
        dc.Fill(ds, "boletas")
        Return ds.Tables("boletas").DefaultView

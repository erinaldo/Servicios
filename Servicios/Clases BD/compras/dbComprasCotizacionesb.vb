Public Class dbComprasCotizacionesb
    ...
    Public Sub Guardar(ByVal pIdProveedor As Integer, ByVal pFecha As String, ByVal pFolio As Integer, ByVal pidMoneda As Integer, ByVal pIva As Double, ByVal pidSucursal As Integer, ByVal pSerie As String)
        ...
        Serie = pSerie
        Comm.CommandText = "insert into tblcomprascotizacionesb(idproveedor,fecha,folio,total,hora,estado,iva,totalapagar,idmoneda,idsucursal,serie,comentario,idUsuarioAlta,fechaAlta,horaAlta,idUsuarioCambio,fechaCambio,horaCambio) values(" + Idproveedor.ToString + ",'" + Fecha + "'," + Folio.ToString + ",0,'" + Format(TimeOfDay, "HH:mm:ss") + "'," + CStr(Estados.SinGuardar) + "," + Iva.ToString + ",0," + idMoneda.ToString + "," + IdSucursal.ToString + ",'" + Replace(Trim(Serie), "'", "''") + "',''," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "'," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "')"
    ...
    Public Function Consulta(ByVal pFecha As String, ByVal pFecha2 As String, ByVal pNombreClave As String, ByVal pEstado As Byte, ByVal pFolio As String, ByVal pIdsucursal As Integer) As DataView
        ...
        Comm.CommandText = "select tblcomprascotizacionesb.idcotizacion,tblcomprascotizacionesb.fecha,concat(tblcomprascotizacionesb.serie,convert(tblcomprascotizacionesb.folio using utf8)) as folio,tblproveedores.clave,tblproveedores.nombre as Proveedor,tblcomprascotizacionesb.totalapagar,if(tblcomprascotizacionesb.estado=4,'Cancelado','Guardado') as sestado  from tblcomprascotizacionesb inner join tblproveedores on tblcomprascotizacionesb.idproveedor=tblproveedores.idproveedor where fecha>='" + pFecha + "' and fecha<='" + pFecha2 + "'"
        ...
        End If
        Comm.CommandText += " order by tblcomprascotizacionesb.fecha desc,tblcomprascotizacionesb.folio desc"
    ...
    Public Function Reporte(ByVal pFecha1 As String, ByVal pFecha2 As String, ByVal pIdSucursal As Integer, ByVal pIdCliente As Integer, ByVal pMostrarEnPesos As Byte, ByVal pSoloCanceladas As Boolean, pIdTipoSucursal As Integer, pIdTipoProv As Integer) As DataView
        ...
        "from tblcomprascotizacionesb v inner join tblproveedores c on v.idproveedor=c.idproveedor inner join tblsucursales s on v.idsucursal=s.idsucursal where v.fecha>='" + pFecha1 + "' and v.fecha<='" + pFecha2 + "'"
        'End If

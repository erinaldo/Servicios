Public Class dbNotasdeCargoComprasDetalles
    ...
    Public Sub LlenaDatos()
        ...
        Comm.CommandText = "select * from tblnotasdecargodetallesc where iddetalle=" + ID.ToString
    ...
    Public Sub Guardar(ByVal pIdCargo As Integer, ByVal pIdinventario As Integer, ByVal pCantidad As Double, ByVal pPrecio As Double, ByVal pIdMoneda As Integer, ByVal pDescripcion As String, ByVal pIva As Double, ByVal pDescuento As Double)
        ...
        'Else
        NuevoConcepto = True
        Comm.CommandText = "insert into tblnotasdecargodetallesc(idcargo,idinventario,cantidad,precio,descripcion,idmoneda,iva,extra,descuento) values(" + IdCargo.ToString + "," + Idinventario.ToString + "," + Cantidad.ToString + "," + Precio.ToString + ",'" + Replace(Descripcion, "'", "''") + "'," + IdMoneda.ToString + "," + Iva.ToString + ",''," + Descuento.ToString + ")"
        Comm.ExecuteNonQuery()
        Comm.CommandText = "select ifnull((select max(iddetalle) from tblnotasdecargodetallesc),0)"
    ...
    Public Sub Modificar(ByVal pID As Integer, ByVal pCantidad As Double, ByVal pPrecio As Double, ByVal pIdMoneda As Integer, ByVal pDescripcion As String, ByVal piva As Double, ByVal pDescuento As Double)
        ...
        Iva = piva
        Descuento = pDescuento
        Comm.CommandText = "update tblnotasdecargodetallesc set precio=" + Precio.ToString + ",idmoneda=" + IdMoneda.ToString + ",cantidad=" + Cantidad.ToString + ",descripcion='" + Replace(Descripcion, "'", "''") + "',iva=" + Iva.ToString + ",descuento=" + Descuento.ToString + " where iddetalle=" + ID.ToString
        Comm.ExecuteNonQuery()
    End Sub

    Public Sub Eliminar(ByVal pID As Integer)
        Comm.CommandText = "delete from tblnotasdecargodetallesc where iddetalle=" + pID.ToString
        Comm.ExecuteNonQuery()
    End Sub
    Public Function Consulta(ByVal pIdVenta As Integer) As DataView
        Dim DS As New DataSet
        Comm.CommandText = "select tvi.iddetalle,tvi.descripcion,tvi.cantidad,tvi.precio,tblmonedas.abreviatura from tblnotasdecargodetallesc tvi inner join tblmonedas on tvi.idmoneda=tblmonedas.idmoneda where tvi.idcargo=" + pIdVenta.ToString
        Dim DA As New MySql.Data.MySqlClient.MySqlDataAdapter(Comm)
        DA.Fill(DS, "tblnotasdecargodetallesc")
        Return DS.Tables("tblnotasdecargodetallesc").DefaultView
    End Function
    Public Function ConsultaReader(ByVal pIdVenta As Integer) As MySql.Data.MySqlClient.MySqlDataReader
        Comm.CommandText = "select tvi.iddetalle,tvi.descripcion,tvi.cantidad,tvi.precio,tblmonedas.abreviatura,tvi.iva,tvi.idmoneda from tblnotasdecargodetallesc tvi inner join tblmonedas on tvi.idmoneda=tblmonedas.idmoneda where tvi.idcargo=" + pIdVenta.ToString

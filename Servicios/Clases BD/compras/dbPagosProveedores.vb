...
Public Class dbPagosProveedores
    ...
    Public Function Guardar(ByVal pTipo As String, ByVal pFolio As String, ByVal pProveedor As String, ByVal pFecha As String, ByVal pFechaCobro As String, ByVal pReferencia As String, ByVal pCantidad As String, ByVal pIVA As String, ByVal pLeyenda As Integer, ByVal pEsCheque As Integer, ByVal pEstado As String, ByVal pBanco As String, pCuentaDestino As String, pIdBancoD As Integer, pBancoDestinoEx As String, pIdMoneda As Integer, pTipodeCambio As Double, pNoCheque As String, pIvaret As Double, pIeps As Double, pISR As Double, pEstraspaso As Byte, pIdCuentaDest As Integer) As Integer
        ...
        Dim id As Integer
        Comm.CommandText = "insert into tblpagoprov (Tipo,Folio, Fecha, FechaCobro,Referencia, Cantidad, IVA, Leyenda, EsCheque, Estado, Banco,idproveedor,cuentadestino,idbancod,bancoorigenex,bancodestinoex,idmoneda,tipodecambio,ivaret,ieps,isr,estraspaso,idcuentadest,idUsuarioAlta,fechaAlta,horaAlta,idUsuarioCambio,fechaCambio,horaCambio) values('" + Replace(pTipo, "'", "''") + "','" + Replace(pFolio, "'", "''") + "','" + Replace(pFecha, "'", "''") + "','" + Replace(pFechaCobro, "'", "''") + "','" + Replace(pReferencia, "'", "''") + "'," + pCantidad + "," + pIVA + ",'" + Replace(pLeyenda, "'", "''") + "','" + Replace(pEsCheque, "'", "''") + "','" + Replace(pEstado, "'", "''") + "'," + pBanco + "," + pProveedor + ",'" + Replace(pCuentaDestino, "'", "''") + "'," + pIdBancoD.ToString + ",'" + pNoCheque.Replace("'", "''") + "','" + pBancoDestinoEx.Replace("'", "''") + "'," + pIdMoneda.ToString + "," + pTipodeCambio.ToString + "," + pIvaret.ToString + "," + pIeps.ToString + "," + pISR.ToString + "," + pEstraspaso.ToString + "," + pIdCuentaDest.ToString + "," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "'," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "')"
        Comm.ExecuteNonQuery()
        Comm.CommandText = "select max(idpagoprov) from tblpagoprov "
        id = Comm.ExecuteScalar

        If pEstraspaso = 1 Then
            Comm.CommandText = "insert into tbldepostito (Fecha,Referencia,Banco,Cantidad,Banco2,comentario,idpagoprov,idUsuarioAlta,fechaAlta,horaAlta,idUsuarioCambio,fechaCambio,horaCambio,fechaconta) values('" + Replace(pFecha, "'", "''") + "','" + Replace(pReferencia, "'", "''") + "',1," + pCantidad + "," + pIdCuentaDest.ToString + ",'TRASPASO'," + id.ToString + "," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "'," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "','" + Replace(pFecha, "'", "''") + "')"
            Comm.ExecuteNonQuery()
        End If
    ...
    Public Sub Modificar(ByVal pId As Integer, ByVal pTipo As String, ByVal pFolio As String, ByVal pProveedor As String, ByVal pFecha As String, ByVal pFechaCobro As String, ByVal pReferencia As String, ByVal pCantidad As String, ByVal pIVA As String, ByVal pLeyenda As Integer, ByVal pEsCheque As Integer, ByVal pEstado As String, ByVal pBanco As String, pCuentaDestino As String, pIdBancoD As Integer, pBancoDestinoEx As String, pIdMoneda As Integer, pTipodeCambio As Double, pNoCheque As String, pIvaret As Double, pIeps As Double, pIsr As Double, pIdCuentaDest As Integer, pEstraspado As Byte)
        'Modificar
        Comm.CommandText = "update tblpagoprov set Tipo='" + Replace(pTipo, "'", "''") + "', Folio='" + Replace(pFolio, "'", "''") + "',Fecha='" + Replace(pFecha, "'", "''") + "',FechaCobro='" + Replace(pFechaCobro, "'", "''") + "',Referencia='" + Replace(pReferencia, "'", "''") + "',Cantidad=" + pCantidad + ",IVA=" + pIVA + ",Leyenda='" + Replace(pLeyenda, "'", "''") + "',EsCheque='" + Replace(pEsCheque, "'", "''") + "',Banco=" + pBanco + ",cuentadestino='" + pCuentaDestino.Replace("'", "''") + "',idbancod=" + pIdBancoD.ToString + ",bancodestinoex='" + pBancoDestinoEx.Replace("'", "''") + "',idmoneda=" + pIdMoneda.ToString + ",tipodecambio=" + pTipodeCambio.ToString + ",bancoorigenex='" + pNoCheque.Replace("'", "''") + "',ivaret=" + pIvaret.ToString + ",ieps=" + pIeps.ToString + ",isr=" + pIsr.ToString + ",idcuentadest=" + pIdCuentaDest.ToString + ", idUsuarioCambio=" + GlobalIdUsuario.ToString() + ", fechaCAmbio='" + DateTime.Now.ToString("yyyy/MM/dd") + "', horaCambio='" + TimeOfDay.ToString("HH:mm:ss") + "', idUsuarioCambio=" + GlobalIdUsuario.ToString() + ", fechaCAmbio='" + DateTime.Now.ToString("yyyy/MM/dd") + "', horaCambio='" + TimeOfDay.ToString("HH:mm:ss") + "' where idPagoProv=" + pId.ToString()
        Comm.ExecuteNonQuery()
        If pEstraspado = 1 Then
            Comm.CommandText = "update tbldepostito set banco2=" + pIdCuentaDest.ToString + ",cantidad=" + pCantidad + ", idUsuarioCambio=" + GlobalIdUsuario.ToString() + ", fechaCAmbio='" + DateTime.Now.ToString("yyyy/MM/dd") + "', horaCambio='" + TimeOfDay.ToString("HH:mm:ss") + "',fecha='" + pFecha + "' where idpagoprov=" + pId.ToString
            Comm.ExecuteNonQuery()
        End If
    ...
    Public Function filtroTodos(pFecha1 As String, pFecha2 As String, pReferencia As String, pIdCuenta As Integer, pFolio As String, pFolioInt As String) As DataView
        ...
        Comm.CommandText = "select idPagoProv,Folio,Fecha,Cantidad,tblproveedores.nombre, FechaCobro,Referencia,tblcuentas.numero,(select nombre from tblbancoscatalogo where idbanco=tblcuentas.banco) from tblpagoprov inner join tblcuentas on tblpagoprov.banco=tblcuentas.idcuenta inner join tblproveedores on tblproveedores.idproveedor=tblpagoprov.idproveedor"
        ...
        If pIdCuenta > 0 Then
            Comm.CommandText += " and tblcuentas.idcuenta=" + pIdCuenta.ToString
        End If
        If pFolio <> "" Then
            Comm.CommandText += " and tblpagoprov.folio like '%" + Replace(pFolio.Trim, "'", "''") + "%'"
        End If
        If pFolioInt <> "" Then
            Comm.CommandText += " and convert(tblpagoprov.idpagoprov using utf8) like '%" + Replace(pFolioInt.Trim, "'", "''") + "%'"
        End If
    ...
    Public Sub Eliminar(ByVal pId As Integer)
        ...
        Comm.ExecuteNonQuery()
        Comm.CommandText = "delete from tbldepostito where idpagoprov=" + pId.ToString
        Comm.ExecuteNonQuery()
    End Sub
    Public Sub Buscar(ByVal pID As Integer)
        ...
        DReader.Close()
    End Sub
    Public Function DaNombreBanco(pidBanco As Integer) As String
        Comm.CommandText = "select ifnull((select nombre from tblbancoscatalogo where idbanco=" + pidBanco.ToString + "),'')"
        Return Comm.ExecuteScalar
    End Function
    Public Function DaNombreMoneda(pidMoneda As Integer) As String
        Comm.CommandText = "select ifnull((select concat(codigo,' ',moneda)  from tblmonedassat where id=" + pidMoneda.ToString + "),'')"
        Return Comm.ExecuteScalar
    End Function

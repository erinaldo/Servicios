...
Public Class dbDescuentos
    ...
    Public Sub Guardar2(ByVal activado As Boolean, ByVal descuento As String, ByVal descripcion As String, ByVal fechai As String, ByVal horai As String, ByVal fechaf As String, ByVal horaf As String, ByVal DataGridview1 As DataGridView, ByVal tipo As String, ByVal pFolio As Integer, ByVal psucursal As Integer)
        ...
        For Each r In DataGridview1.Rows
            Comm.CommandText = "insert into tbldescuentos (activado, descuento, descripcion, fechai, horai, fechaf, horaf, productos,tipoDescuento,Folio,FechaInicioC,FechaFinalC,sucursal,idUsuarioAlta,fechaAlta,horaAlta,idUsuarioCambio,fechaCambio,horaCambio) values(" + activado.ToString() + " , '" + descuento.ToString() + "' ,'" + Replace(descripcion, "'", "''") + "','" + Replace(fechai, "'", "''") + "','" + Replace(horai, "'", "''") + "','" + Replace(fechaf, "'", "''") + "','" + Replace(horaf, "'", "''") + "','" + Replace(r.Cells(1).Value, "'", "''") + "','" + Replace(tipo, "'", "''") + "','" + pFolio.ToString() + "','" + fechai + " " + horai + "','" + fechaf + " " + horaf + "'," + psucursal.ToString + "," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "'," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "');"
            Comm.ExecuteNonQuery()
        Next

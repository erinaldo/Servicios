...
Public Class dbEmpenios
    ...
    End Function
    Public Function CreaRenovado(pid As Integer, pFecha As String, pCantidad As Double) As Integer
        Dim CuantosRenglones As Integer
        Comm.CommandText = "insert into tblempenios(fecha,folio,hora,estado,total,idsucursal,serie,idcaja,idvendedor,comentario,tipo,idcliente, diasPrestamos,A1a30,A31a60,A61a90,A90mas,B1a30,B31a60,B61a90,B90mas,fechaContrato,pagado, TotalAux,idforma,Adjudicado,tipoEmpenio, renovado,idempenioant) "
        Comm.CommandText += "select  '" + pFecha + "',folio,'" + Date.Now.ToString("HH:mm:ss") + "',estado," + pCantidad.ToString + ",idsucursal,serie,idcaja,idvendedor,comentario,tipo,idcliente, diasPrestamos,A1a30,A31a60,A61a90,A90mas,B1a30,B31a60,B61a90,B90mas,'" + pFecha + "',0," + pCantidad.ToString + ",idforma,Adjudicado,tipoEmpenio,1," + pid.ToString + " from tblempenios where idmovimiento=" + pid.ToString
    ...
    Public Sub Guardar(ByVal pIdCliente As Integer, ByVal pFecha As String, ByVal pFolio As Integer, ByVal pTotal As Double, ByVal pidSucursal As Integer, ByVal pSerie As String, ByVal pidCaja As Integer, ByVal pIdVendedor As Integer, ByVal pTipo As Byte, ByVal pComentario As String, ByVal pDiasPrestamos As Integer, ByVal Pidforma As Integer, ByVal ptipoEmpenio As Integer)
        ...
        Comm.CommandText = "insert into tblempenios(fecha,folio,hora,estado,total,idsucursal,serie,idcaja,idvendedor,comentario,tipo,idcliente, diasPrestamos,A1a30,A31a60,A61a90,A90mas,B1a30,B31a60,B61a90,B90mas,fechaContrato,pagado, TotalAux,idforma,Adjudicado,tipoEmpenio, renovado,idempenioant) values('" + Replace(Fecha, "'", "''") + "'," + Folio.ToString + ",'" + Format(TimeOfDay, "HH:mm:ss") + "'," + CStr(Estados.SinGuardar) + "," + Total.ToString() + "," + IdSucursal.ToString + ",'" + Replace(Trim(Serie), "'", "''") + "'," + IdCaja.ToString + "," + IdVendedor.ToString + ",'" + Replace(Comentario, "'", "''") + "'," + Tipo.ToString + "," + idCliente.ToString + "," + diasPrestamo.ToString + ",0,0,0,0,0,0,0,0,'" + Replace(Fecha, "'", "''") + "',0," + Total.ToString + "," + Pidforma.ToString + ", 0," + tipoEmpenio.ToString + ",0,0);"
        'Comm.ExecuteNonQuery()
        Comm.CommandText += "select max(idmovimiento) from tblempenios;"
        ID = Comm.ExecuteScalar
    End Sub
    ...
    Public Sub Modificar(ByVal pID As Integer, ByVal pIdCliente As Integer, ByVal pFecha As String, ByVal pFolio As Integer, ByVal pTotal As Double, ByVal pidSucursal As Integer, ByVal pSerie As String, ByVal pidCaja As Integer, ByVal pIdVendedor As Integer, ByVal pTipo As Byte, ByVal pComentario As String, ByVal pEstado As Integer, ByVal pDiasPrestamos As Integer, ByVal PIdForma As Integer, ByVal ptipoEmpenio As Integer)
        ...
        IdForma = PIdForma
        tipoEmpenio = ptipoEmpenio
        Comm.CommandText = "update tblempenios set fecha='" + Replace(Fecha, "'", "''") + "', fechaContrato='" + Replace(Fecha, "'", "''") + "',folio=" + Folio.ToString + ",estado=" + Estado.ToString + ",total=" + Total.ToString + ",TotalAux=" + Total.ToString + ",serie='" + Replace(Trim(Serie), "'", "''") + "',idvendedor=" + IdVendedor.ToString + ",comentario='" + Replace(Comentario, "'", "''") + "',tipo=" + Tipo.ToString + ",idcaja=" + IdCaja.ToString + ",idcliente=" + idCliente.ToString + ",diasPrestamos=" + diasPrestamo.ToString + ",idforma=" + PIdForma.ToString + ", tipoEmpenio=" + tipoEmpenio.ToString + " where idmovimiento=" + ID.ToString
        Comm.ExecuteNonQuery()
    End Sub
    ...
    Public Function Consulta(ByVal pFecha As String, ByVal pFecha2 As String, ByVal pEstado As Byte, ByVal pFolio As String, ByVal pidSucursal As Integer, ByVal pidCaja As Integer, ByVal PCliente As String) As DataView
        ...
        Comm.CommandText = "select tblempenios.idmovimiento,tblempenios.fechaContrato,concat(tblempenios.serie,convert(tblempenios.folio using utf8)),concat(tblclientes.clave,' ',tblclientes.nombre),tblempenios.totalAux as total,tblcajas.nombre as caja,case tblempenios.estado when 2 then 'Pendiente' when 3 then 'Guardado' when 4 then 'Cancelada' end  as sestado from tblempenios inner join tblcajas on tblempenios.idcaja=tblcajas.idcaja inner join tblclientes on tblempenios.idcliente=tblclientes.idcliente where tblempenios.fecha>='" + pFecha + "' and tblempenios.fecha<='" + pFecha2 + "'"
        If PCliente <> "" Then

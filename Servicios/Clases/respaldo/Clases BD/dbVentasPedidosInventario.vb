Public Class dbVentasPedidosInventario
    ...
    Public Sub LlenaDatos()
        ...
        Comm.CommandText = "select * from tblventaspedidosinventario where iddetalle=" + ID.ToString
    ...
    End Sub
    Public Sub Guardar(ByVal pIdPedido As Integer, ByVal pIdinventario As Integer, ByVal pCantidad As Double, ByVal pPrecio As Double, ByVal pIdMoneda As Integer, ByVal pDescripcion As String, ByVal pIva As Double, ByVal pDescuento As Double, ByVal pIdVariante As Integer, ByVal pIEPS As Double, ByVal pIVARetenido As Double)
        Dim CTemp As Double
        Dim PTemp As Double
    ...
    Public Sub Modificar(ByVal pID As Integer, ByVal pCantidad As Double, ByVal pPrecio As Double, ByVal pIdMoneda As Integer, ByVal pDescripcion As String, ByVal pIva As Double, ByVal pDescuento As Double, ByVal pIEPS As Double, ByVal pIVARetenido As Double)
        ...
        Else
            PrecioOriginal = Precio / Cantidad
        End If
        Comm.CommandText = "update tblventaspedidosinventario set precio=" + Precio.ToString + ",idmoneda=" + IdMoneda.ToString + ",cantidad=" + Cantidad.ToString + ",descripcion='" + Replace(Descripcion, "'", "''") + "',iva=" + Iva.ToString + ",descuento=" + Descuento.ToString + ",preciooriginal=" + Precio.ToString + ",IEPS=" + IEPS.ToString() + ",IVARetenido=" + IVARetenido.ToString() + " where iddetalle=" + ID.ToString
        Comm.ExecuteNonQuery()
    End Sub
    Public Sub Eliminar(ByVal pID As Integer)
        Comm.CommandText = "delete from tblventaspedidosinventario where iddetalle=" + pID.ToString
        Comm.ExecuteNonQuery()
    End Sub
    Public Function Consulta(ByVal pIdPedido As Integer) As DataView
        Dim DS As New DataSet
        Comm.CommandText = "select tvi.iddetalle,tblinventario.clave,tvi.descripcion,tvi.cantidad,tvi.precio,tblmonedas.abreviatura from tblventaspedidosinventario tvi inner join tblinventario on tvi.idinventario=tblinventario.idinventario inner join tblmonedas on tvi.idmoneda=tblmonedas.idmoneda where tvi.idpedido=" + pIdPedido.ToString
        Dim DA As New MySql.Data.MySqlClient.MySqlDataAdapter(Comm)
        DA.Fill(DS, "tblventaspedidosinventario")
        Return DS.Tables("tblventaspedidosinventario").DefaultView
    End Function
    Public Function ConsultaReader(ByVal pIdPedido As Integer) As MySql.Data.MySqlClient.MySqlDataReader
        Comm.CommandText = "select tvi.iddetalle,tblinventario.clave,tvi.descripcion,tvi.cantidad,tvi.precio,tblmonedas.abreviatura,tbltiposcantidades.abreviatura as tipocantidad,tblproductos.clave as pclave,tvi.idinventario,tvi.idvariante,tvi.descuento,tvi.IEPS,tvi.IVARetenido,tvi.iva from tblventaspedidosinventario tvi inner join tblinventario on tvi.idinventario=tblinventario.idinventario inner join tblmonedas on tvi.idmoneda=tblmonedas.idmoneda inner join tbltiposcantidades on tblinventario.tipocontenido=tbltiposcantidades.idtipocantidad inner join tblproductosvariantes on tvi.idvariante=tblproductosvariantes.idvariante inner join tblproductos on tblproductos.idproducto=tblproductosvariantes.idproducto where tvi.idpedido=" + pIdPedido.ToString
        Return Comm.ExecuteReader
    End Function
    Public Function ConsultaReaderIVA(ByVal pIdPedido As Integer) As MySql.Data.MySqlClient.MySqlDataReader
        Comm.CommandText = "select tvi.iddetalle,tblinventario.clave,tvi.descripcion,tvi.cantidad,tvi.precio*(1+(tvi.iva+tvi.ieps-tvi.ivaretenido)/100) as precio,tblmonedas.abreviatura,tbltiposcantidades.abreviatura as tipocantidad,tblproductos.clave as pclave,tvi.idinventario,tvi.idvariante,tvi.descuento,tvi.iva from tblventaspedidosinventario tvi inner join tblinventario on tvi.idinventario=tblinventario.idinventario inner join tblmonedas on tvi.idmoneda=tblmonedas.idmoneda inner join tbltiposcantidades on tblinventario.tipocontenido=tbltiposcantidades.idtipocantidad inner join tblproductosvariantes on tvi.idvariante=tblproductosvariantes.idvariante inner join tblproductos on tblproductos.idproducto=tblproductosvariantes.idproducto where tvi.idpedido=" + pIdPedido.ToString
        Return Comm.ExecuteReader
    End Function
    ...
    Public Sub CambiaPrecio(ByVal pId As Integer, ByVal pPrecio As Double, ByVal pTipoRedondeo As Byte, ByVal pCantidadDecimales As Byte, ByVal pDescuento As Double)
        ...
        End If
    End Sub
    Public Sub SeparaKit(ByVal pIdPedido As Integer, ByVal pIdinventario As Integer, ByVal pCantidad As Double, ByVal pPrecio As Double, ByVal pIdMoneda As Integer, ByVal pDescripcion As String, ByVal pIva As Double, ByVal pDescuento As Double, ByVal pIdVariante As Integer, ByVal pIesp As Double, ByVal pivaRet As Double)
        Dim STR As String
        Idinventario = pIdinventario
        Cantidad = pCantidad
        Precio = pPrecio
        ...
        STR = "insert into tblventaspedidosinventario(idpedido,idinventario,cantidad,precio,descripcion,idmoneda,iva,extra,descuento,idvariante,surtido,preciooriginal,ieps,ivaretenido) values(" + IdPedido.ToString + "," + Idinventario.ToString + ",0,0,'" + Replace(Descripcion, "'", "''") + "'," + IdMoneda.ToString + "," + Iva.ToString + ",''," + Descuento.ToString + "," + IdVariante.ToString + ",0," + Precio.ToString + "," + pIesp.ToString + "," + pivaRet.ToString + ");"
        STR += "insert into tblventaspedidosinventario(idpedido,idinventario,cantidad,precio,descripcion,idmoneda,iva,extra,descuento,idvariante,surtido,preciooriginal,ieps,ivaretenido) select " + IdPedido.ToString + ",tblinventariodetalles.idinventario,tblinventariodetalles.cantidad,(select precio from tblinventarioprecios where idinventario=tblinventariodetalles.idinventario and idlista=1 limit 1)*tblinventariodetalles.cantidad,(select nombre from tblinventario where idinventario=tblinventariodetalles.idinventario)," + IdMoneda.ToString + ",(select iva from tblinventario where idinventario=tblinventariodetalles.idinventario),''," + Descuento.ToString + "," + IdVariante.ToString + ",0,(select precio from tblinventarioprecios where idinventario=tblinventariodetalles.idinventario and idlista=1 limit 1),(select ieps from tblinventario where idinventario=tblinventariodetalles.idinventario),(select ivaretenido from tblinventario where idinventario=tblinventariodetalles.idinventario) from tblinventariodetalles inner join tblinventario on tblinventario.idinventario=tblinventariodetalles.idinventariop  where tblinventariodetalles.idinventariop=" + pIdinventario.ToString + ";"
        Comm.CommandText = STR
        Comm.ExecuteNonQuery()
    End Sub
    Public Function BuscaridInventario(ByVal pId As Integer) As Integer
        Dim Resultado As Integer = 0
        Comm.CommandText = "select idinventario from tblventaspedidosinventario where iddetalle=" + pId.ToString
    ...
    Public Sub GuardarDescuento(ByVal pIdPedido As Integer, ByVal pIdinventario As Integer, ByVal pCantidad As Double, ByVal pPrecio As Double, ByVal pIdMoneda As Integer, ByVal pDescripcion As String, ByVal pIva As Double, ByVal pDescuento As Double, ByVal pIdVariante As Integer, ByVal pIEPS As Double, ByVal pIvaRetenido As Double)
        ...
        If IdTemp <> 0 Then
            ...
            Comm.CommandText = "update tblventaspedidosinventario set cantidad=" + pCantidad.ToString + ",precio=" + pPrecio.ToString + " where iddetalle=" + IdTemp.ToString
            Comm.ExecuteNonQuery()
            'ID = IdTemp
            LlenaDatos()
            'NuevoConcepto = False
        Else
            NuevoConcepto = True
            Comm.CommandText = "insert into tblventaspedidosinventario(idpedido,idinventario,cantidad,precio,descripcion,idmoneda,iva,extra,descuento,idvariante,surtido,preciooriginal,IEPS, IVARetenido) values(" + pIdPedido.ToString + "," + pIdinventario.ToString + "," + pCantidad.ToString + "," + pPrecio.ToString + ",'" + Replace(pDescripcion, "'", "''") + "'," + pIdMoneda.ToString + "," + pIva.ToString + ",''," + pDescuento.ToString + "," + pIdVariante.ToString + ",0," + pPrecio.ToString + "," + pIEPS.ToString + "," + pIvaRetenido + ")"
            Comm.ExecuteNonQuery()
    ...
    Public Function UltomoRegistro() As Integer
        Dim id As Integer
        Comm.CommandText = "select max(iddetalle) FROM tblventaspedidosinventario"

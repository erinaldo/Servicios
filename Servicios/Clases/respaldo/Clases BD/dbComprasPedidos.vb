Public Class dbComprasPedidos
    ...
    Public Sub Guardar(ByVal pIdProveedor As Integer, ByVal pFecha As String, ByVal pFolio As Integer, ByVal pidMoneda As Integer, ByVal pIva As Double, ByVal pidSucursal As Integer, ByVal pSerie As String)
        ...
        Serie = pSerie
        Comm.CommandText = "insert into tblcompraspedidos(idproveedor,fecha,folio,total,hora,estado,iva,totalapagar,idmoneda,idsucursal,serie,comentario) values(" + Idproveedor.ToString + ",'" + Fecha + "'," + Folio.ToString + ",0,'" + Format(TimeOfDay, "HH:mm:ss") + "'," + CStr(Estados.SinGuardar) + "," + Iva.ToString + ",0," + idMoneda.ToString + "," + IdSucursal.ToString + ",'" + Replace(Trim(Serie), "'", "''") + "','')"
    ...
    Public Function Consulta(ByVal pFecha As String, ByVal pFecha2 As String, ByVal pNombreClave As String, ByVal pEstado As Byte, ByVal pFolio As String, ByVal pIdSucursal As Integer) As DataView
        ...
        Comm.CommandText = "select tblcompraspedidos.idpedido,tblcompraspedidos.fecha,tblcompraspedidos.folio,tblproveedores.clave,tblproveedores.nombre as Proveedor,tblcompraspedidos.total  from tblcompraspedidos inner join tblproveedores on tblcompraspedidos.idproveedor=tblproveedores.idproveedor where fecha>='" + pFecha + "' and fecha<='" + pFecha2 + "'"
        If pNombreClave <> "" Then
            Comm.CommandText += " and concat(tblproveedores.clave,tblproveedores.nombre) like '%" + Replace(pNombreClave, "'", "''") + "%'"
        End If
        If pFolio <> "" Then
            Comm.CommandText += " and concat(tblcompraspedidos.serie,convert(tblcompraspedidos.folio using utf8)) like '%" + Replace(pFolio, "'", "''") + "%'"
        End If
        ...
        If pIdSucursal > 0 Then
            Comm.CommandText += " and tblcompraspedidos.idsucursal=" + pIdSucursal.ToString
        End If
        Comm.CommandText += " order by tblcompraspedidos.fecha,tblcompraspedidos.folio"
    ...
    Public Function Reporte(ByVal pFecha1 As String, ByVal pFecha2 As String, ByVal pIdSucursal As Integer, ByVal pIdCliente As Integer, ByVal pMostrarEnPesos As Byte, ByVal pSoloCanceladas As Boolean) As DataView
        ...
        "from tblcompraspedidos v inner join tblproveedores c on v.idproveedor=c.idproveedor where v.fecha>='" + pFecha1 + "' and v.fecha<='" + pFecha2 + "'"
        'End If

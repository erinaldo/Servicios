Public Class dbNominasIncapacidades
    ...
    Public Sub Guardar(ByVal pIdNomina As Integer, ByVal pTipo As Byte, ByVal pDias As Integer, ByVal pDescuento As Double)
        ...
        Comm.CommandText = "select ifnull((select max(iddetalle) from tblnominaincapacidades),0)"
    ...
    Public Function ConsultaReader(ByVal pIdVenta As Integer) As MySql.Data.MySqlClient.MySqlDataReader
        Comm.CommandText = "select tvi.iddetalle,case tvi.tipoincapacidad when 0 then 'Riesgo de trabajo' when 1 then 'Enfermedad en general' when 2 then 'Maternidad' end as tipod,tvi.dias,tvi.descuento,tvi.tipoincapacidad from tblnominaincapacidades as tvi where tvi.idnomina=" + pIdVenta.ToString

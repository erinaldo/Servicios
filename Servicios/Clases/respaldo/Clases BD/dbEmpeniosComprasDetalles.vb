Public Class dbEmpeniosComprasDetalles
    ...
    Public Sub LlenaDatos()
        ...
        Comm.CommandText = "select * from tblempeniosgastosdetalles where idDetalle=" + ID.ToString
    ...
    Public Sub Guardar(ByVal pidMovimiento As Integer, ByVal pDescripcion As String, ByVal pPrecio As Double, ByVal pClasificacion As Integer, ByVal pKilates As Double, ByVal pPeso As Double, ByVal pEvaluo As Double, ByVal pTipo As Integer)
        ...
        peso = pPeso
        evaluo = pEvaluo
        Comm.CommandText = "insert into tblempeniosgastosdetalles(idMovimiento,descripcion, precio,clasificacion,kilates,peso,evaluo,tipo) values(" + idMovimiento.ToString + ",'" + Replace(descripcion, "'", "''") + "'," + precio.ToString + "," + clasificacion.ToString() + "," + kilates.ToString + "," + peso.ToString + "," + evaluo.ToString + " , " + pTipo.ToString + ");"
        Comm.CommandText += "select ifnull((select max(idDetalle) from tblempeniosgastosdetalles),0);"
    ...
    Public Sub Modificar(ByVal pID As Integer, ByVal pidMovimiento As Integer, ByVal pDescripcion As String, ByVal pPrecio As Double, ByVal pClasificacion As Integer, ByVal pKilates As Double, ByVal pPeso As Double, ByVal pEvaluo As Double, ByVal pTipo As Integer)
        ...
        peso = pPeso
        evaluo = pEvaluo
        Comm.CommandText = "update tblempeniosgastosdetalles set descripcion='" + Replace(descripcion, "'", "''") + "',Precio=" + precio.ToString() + ",clasificacion=" + clasificacion.ToString + ",kilates=" + kilates.ToString + ",peso=" + peso.ToString + ",evaluo=" + evaluo.ToString + ",tipo=" + pTipo.ToString + " where idDetalle=" + ID.ToString
        Comm.ExecuteNonQuery()
    End Sub
    Public Sub Eliminar(ByVal pID As Integer)
        Comm.CommandText = "delete from tblempeniosgastosdetalles where idDetalle=" + pID.ToString
        Comm.ExecuteNonQuery()
    End Sub
    ...
    Public Function impresionRecibo(ByVal pIdMovimiento As Integer) As DataTable
        Dim DS As New DataSet
        Comm.CommandText = "select CONCAT(if (tipo=0,'Oro','Plata'),'  ',descripcion,'  ', CASE kilates WHEN 0 THEN '0k' WHEN 1 THEN '8k' WHEN 2 THEN '10k' WHEN 3 THEN '14k' WHEN 4 THEN '18k' WHEN 5 THEN '24k' ELSE 'otro/desconocido' End,'  ',peso,'gr.') as concepto, precio as Precio from tblempeniosgastosdetalles  where idMovimiento=" + pIdMovimiento.ToString
    ...
    Public Function DaTotal(ByVal pidMovimiento As Integer) As Double
        ...
        '+++++++++++++++++++++++++++++++++++++++Total por Articulos ++++++++++++++++++++++++++++++++++++
        Comm.CommandText = "select idDetalle from tblempeniosgastosdetalles where idMovimiento=" + pidMovimiento.ToString
        DReader = Comm.ExecuteReader
        While DReader.Read
        ...
        While Cont <= IDs.Count
            Comm.CommandText = "select precio from tblempeniosgastosdetalles where idDetalle=" + IDs.Item(Cont).ToString

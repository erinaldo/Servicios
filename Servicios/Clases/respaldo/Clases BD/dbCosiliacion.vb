...
Public Class dbCosiliacion
    ...
    Public Function filtroPagoProv(ByVal fInicio As String, ByVal fFinal As String, ByVal pBanco As String) As DataView
        Dim DS As New DataSet
        Comm.CommandText = "select 1 as seleccion,idPagoProv as ID,Folio,(select nombre from tblproveedores where tblproveedores.idproveedor=tblpagoprov.idproveedor) Proveedor, Fecha, FechaCobro as Cobro,Referencia,(select numero from tblcuentas where idCuenta=tblpagoprov.Banco)as NoCuenta,(select nombre from tblbancoscatalogo inner join tblcuentas on tblbancoscatalogo.idBanco=tblCuentas.Banco where tblCuentas.idCuenta=tblpagoprov.Banco)as Banco, Cantidad,Tipo, IVA, Leyenda, EsCheque, Estado from tblpagoprov  where idPagoProv>0 "
        If fInicio <> "" Then
    ...
    End Function
    'Especifico
    Public Function saldoInicialAmbos(ByVal fInicio As String, ByVal pBanco As String) As DataTable 'error
        Dim DS As New DataSet
        Comm.CommandText = "select Cantidad from tbldepostito where  Banco2='" + pBanco.ToString() + "' And DATE(Fecha) between' 0000/00/00' AND '" + fInicio.ToString() + "'"
        Dim DA As New MySql.Data.MySqlClient.MySqlDataAdapter(Comm)
        DA.Fill(DS, "tbldepostito")
        Return DS.Tables("tbldepostito")
    End Function
    Public Function saldoInicialAmbosPagos(ByVal fInicio As String, ByVal pBanco As String) As DataTable 'error
        Dim DS As New DataSet
        Comm.CommandText = "select Cantidad from tblpagoprov where Banco='" + pBanco + "' And DATE(Fechacobro) between' 0000/00/00' AND '" + fInicio.ToString() + "'"
        Dim DA As New MySql.Data.MySqlClient.MySqlDataAdapter(Comm)
        DA.Fill(DS, "tblpagoprov")
        Return DS.Tables("tblpagoprov")
    End Function

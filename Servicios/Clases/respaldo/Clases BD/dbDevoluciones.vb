Public Class dbDevoluciones
    ...
    Private Sub LlenaDatos()
        ...
        Comm.CommandText = "select * from tbldevoluciones where iddevolucion=" + ID.ToString
    ...
    Public Function Consulta(ByVal pFecha As String, ByVal pFecha2 As String, Optional ByVal pNombreClave As String = "", Optional ByVal pFolio As String = "", Optional ByVal pEstado As Byte = 0, Optional ByVal pCredido As Byte = 200) As DataView
        ...
        ' End If
        Comm.CommandText += " order by tbldevoluciones.serie,tbldevoluciones.folio,tbldevoluciones.fecha"
...
"tblventasinventario.tipocantidadm,tblventasinventario.cantidad,tblventasinventario.cantidadm,tblventasinventario.ieps,tblventasinventario.ivaretenido from tblventasinventario inner join tblinventario on tblventasinventario.idinventario=tblinventario.idinventario where tblinventario.inventariable=1 and idventa=" + pIdDocumento.ToString + " and (tblventasinventario.cantidad-(ifnull((select sum(tbldevolucionesdetalles.cantidad) from tbldevolucionesdetalles inner join tbldevoluciones on tbldevolucionesdetalles.iddevolucion=tbldevoluciones.iddevolucion where tbldevoluciones.idventa=tblventasinventario.idventa and tbldevolucionesdetalles.idinventario=tblventasinventario.idinventario and tbldevoluciones.estado=3),0)))>0"
    ...
    Public Sub ModificaInventario(ByVal pId As Integer)
        ...
        Dim IDs As New Collection
        Dim Cont As Integer = 1
        Comm.CommandText = "select iddetalle from tbldevolucionesdetalles where iddevolucion=" + ID.ToString
        DReader = Comm.ExecuteReader
        While DReader.Read()
            IDs.Add(DReader("iddetalle"))
        End While
        ...
        While Cont <= IDs.Count
            Comm.CommandText = "select idinventario,idvariante,cantidad,idalmacen from tbldevolucionesdetalles where iddetalle=" + IDs(Cont).ToString
            DReader = Comm.ExecuteReader
            If DReader.Read() Then
                ...
                If iIdInventario > 1 Then
                    I.MovimientoDeInventario(iIdInventario, iCantidad, 0, dbInventario.TipoMovimiento.Alta, iIdAlmacen)
                End If
    ...
    Public Function Reporte(ByVal pFecha1 As String, ByVal pFecha2 As String, ByVal pIdSucursal As Integer, ByVal pIdCliente As Integer, ByVal pSoloCanceladas As Boolean, ByVal pMostrarEnPesos As Byte, ByVal pidInventario As Integer, ByVal pSerie As String, pIdAlmacen As Integer) As DataView
        Dim DS As New DataSet
        If pMostrarEnPesos = 0 Then
            Comm.CommandText = "select td.iddevolucion,td.fecha,td.serie,td.folio,tdd.cantidad,if(tdd.idmoneda=2,tdd.precio,tdd.precio*td.tipodecambio) as precio,td.estado,tdd.iva,tdd.ieps,tdd.ivaretenido,if(td.idventa<>0,'FACTURAS','REMISIONES') as tipodev," + _
            "if(tdd.idinventario>1,(select clave from tblinventario where tblinventario.idinventario=tdd.idinventario),(select clave from tblproductos inner join tblproductosvariantes on tblproductos.idproducto=tblproductosvariantes.idproducto where tblproductosvariantes.idvariante=tdd.idvariante)) as clave," + _
            "if(tdd.idinventario>1,(select nombre from tblinventario where tblinventario.idinventario=tdd.idinventario),(select tblproductosvariantes.nombre from tblproductos inner join tblproductosvariantes on tblproductos.idproducto=tblproductosvariantes.idproducto where tblproductosvariantes.idvariante=tdd.idvariante)) as nombre " + _
            " from tbldevoluciones as td inner join tbldevolucionesdetalles as tdd on td.iddevolucion=tdd.iddevolucion where td.fecha>='" + pFecha1 + "' and td.fecha<='" + pFecha2 + "' "
        Else
            Comm.CommandText = "select td.iddevolucion,td.fecha,td.serie,td.folio,tdd.cantidad,tdd.precio as precio,td.estado,tdd.iva,tdd.ieps,tdd.ivaretenido,if(td.idventa<>0,'FACTURAS','REMISIONES') as tipodev," + _
            "if(tdd.idinventario>1,(select clave from tblinventario where tblinventario.idinventario=tdd.idinventario),(select clave from tblproductos inner join tblproductosvariantes on tblproductos.idproducto=tblproductosvariantes.idproducto where tblproductosvariantes.idvariante=tdd.idvariante)) as clave," + _
            "if(tdd.idinventario>1,(select nombre from tblinventario where tblinventario.idinventario=tdd.idinventario),(select tblproductosvariantes.nombre from tblproductos inner join tblproductosvariantes on tblproductos.idproducto=tblproductosvariantes.idproducto where tblproductosvariantes.idvariante=tdd.idvariante)) as nombre " + _
            " from tbldevoluciones as td inner join tbldevolucionesdetalles as tdd on td.iddevolucion=tdd.iddevolucion where td.fecha>='" + pFecha1 + "' and td.fecha<='" + pFecha2 + "' "
        End If

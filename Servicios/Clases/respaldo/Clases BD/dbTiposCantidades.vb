Public Class dbTiposCantidades
    ...
    Public Sub New(ByVal pID As Integer, ByVal Conexion As MySql.Data.MySqlClient.MySqlConnection)
        ...
        DReader.Close()
    End Sub
    Public Sub Guardar(ByVal pNombre As String, ByVal pAbreviatura As String, ByVal pUsaBascula As Byte)
        Nombre = pNombre
        Abreviatura = pAbreviatura
        UsaBascula = pUsaBascula
        Comm.CommandText = "insert into tbltiposcantidades(nombre,abreviatura,usabascula) values('" + Replace(Nombre, "'", "''") + "','" + Replace(Abreviatura, "'", "''") + "'," + UsaBascula.ToString + ")"
        Comm.ExecuteNonQuery()
        Comm.CommandText = "select max(idtipocantidad) from tbltiposcantidades"
        ID = Comm.ExecuteScalar
    End Sub
    Public Sub Modificar(ByVal pID As Integer, ByVal pNombre As String, ByVal pAbreviatura As String, ByVal pUsaBascula As Byte)
        ID = pID
        Nombre = pNombre
        Abreviatura = pAbreviatura
        UsaBascula = pUsaBascula
        Comm.CommandText = "update tbltiposcantidades set nombre='" + Replace(Nombre, "'", "''") + "',abreviatura='" + Replace(Abreviatura, "'", "''") + "',usabascula=" + UsaBascula.ToString + " where idtipocantidad=" + ID.ToString
        Comm.ExecuteNonQuery()
    End Sub

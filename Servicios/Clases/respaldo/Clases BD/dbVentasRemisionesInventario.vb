Public Class dbVentasRemisionesInventario
    ...
    End Sub
    Public Sub Guardar(ByVal pIdVenta As Integer, ByVal pIdinventario As Integer, ByVal pCantidad As Double, ByVal pPrecio As Double, ByVal pIdMoneda As Integer, ByVal pDescripcion As String, ByVal pIdAlmacen As Integer, ByVal pIva As Double, ByVal pDescuento As Double, ByVal pIdVariante As Integer, ByVal pIdServicio As Integer, ByVal pIEPS As Double, ByVal pIVARetenido As Double, pCantidadM As Double, pTipoCantidadM As Integer)
        ...
        '    Comm.CommandText = "select ifnull((select iddetalle from tblventasremisionesinventario where idremision=" + IdRemision.ToString + " and idinventario=" + Idinventario.ToString + " limit 1),0)"
        '    IdTemp = Comm.ExecuteScalar
        'Else
        '    Idinventario = 1
        'End If
        'If IdVariante <> 0 Then
        '    Comm.CommandText = "select ifnull((select iddetalle from tblventasremisionesinventario where idremision=" + IdRemision.ToString + " and idvariante=" + IdVariante.ToString + " limit 1),0)"
        '    IdTemp = Comm.ExecuteScalar
        'Else
        '    IdVariante = 1
        'End If
        'If IdServicio <> 0 Then
        '    Comm.CommandText = "select ifnull((select iddetalle from tblventasremisionesinventario where idremision=" + IdRemision.ToString + " and idservicio=" + IdServicio.ToString + " limit 1),0)"
        '    IdTemp = Comm.ExecuteScalar
        'End If
        'IdTemp = 0
        'If IdTemp <> 0 Then
        ...
        '    Comm.CommandText = "update tblventasremisionesinventario set cantidad=" + Cantidad.ToString + ",precio=" + Precio.ToString + " where iddetalle=" + IdTemp.ToString
        ...
        'Else
        NuevoConcepto = True
        Comm.CommandText = "insert into tblventasremisionesinventario(idremision,idinventario,cantidad,precio,descripcion,idmoneda,idalmacen,iva,extra,descuento,idvariante,idservicio,surtido,preciooriginal,IEPS,IVARetenido,cantidadm,tipocantidadm) values(" + IdRemision.ToString + "," + Idinventario.ToString + "," + Cantidad.ToString + "," + Precio.ToString + ",'" + Replace(Descripcion, "'", "''") + "'," + IdMoneda.ToString + "," + IdAlmacen.ToString + "," + Iva.ToString + ",''," + Descuento.ToString + ",1," + IdServicio.ToString + ",0," + Precio.ToString + " , " + IEPS.ToString() + " , " + IVARetenido.ToString() + "," + CantidadM.ToString + "," + TipoCantidadM.ToString + ");"
        'Comm.ExecuteNonQuery()
        ...
        'Iva = pIva
        'Descuento = pDescuento
        'Comm.CommandText = "select ifnull((select max(cantidad) from tblventasremisionesinventario where idremision=" + IdRemision.ToString + " and idinventario=" + Idinventario.ToString + "),-1)"
        'CTemp = Comm.ExecuteScalar

        'If CTemp > -1 Then
        '    Comm.CommandText = "select max(precio) from tblventasremisionesinventario where idremision=" + IdRemision.ToString + " and idinventario=" + Idinventario.ToString
        '    PTemp = Comm.ExecuteScalar
        '    If PTemp <> 0 Then
        '        Precio = PTemp / CTemp
        '    Else
        ...
        '    Cantidad += CTemp
        '    Precio = Precio * Cantidad
        '    Comm.CommandText = "update tblventasremisionesinventario set cantidad=" + Cantidad.ToString + ",precio=" + Precio.ToString + " where idinventario=" + Idinventario.ToString + " and idremision=" + IdRemision.ToString
        '    Comm.ExecuteNonQuery()
        '    Comm.CommandText = "select iddetalle from tblventasremisionesinventario where idinventario=" + Idinventario.ToString + " and idremision=" + IdRemision.ToString
        '    ID = Comm.ExecuteScalar
        '    LlenaDatos()
        '    NuevoConcepto = False
        'Else
        '    NuevoConcepto = True
        '    Comm.CommandText = "insert into tblventasremisionesinventario(idremision,idinventario,cantidad,precio,descripcion,idmoneda,idalmacen,iva,extra,descuento) values(" + IdRemision.ToString + "," + Idinventario.ToString + "," + Cantidad.ToString + "," + Precio.ToString + ",'" + Replace(Descripcion, "'", "''") + "'," + IdMoneda.ToString + "," + IdAlmacen.ToString + "," + Iva.ToString + ",''," + Descuento.ToString + ")"
        '    Comm.ExecuteNonQuery()
        '    Comm.CommandText = "select ifnull((select max(iddetalle) from tblventasremisionesinventario),0)"
        '    ID = Comm.ExecuteScalar
        'End If
    ...
    End Sub
    Public Sub Modificar(ByVal pID As Integer, ByVal pCantidad As Double, ByVal pPrecio As Double, ByVal pIdMoneda As Integer, ByVal pDescripcion As String, ByVal pIva As Double, ByVal pDescuento As Double, ByVal pIEPS As Double, ByVal pIVARetenido As Double, pCantidadM As Double, pTipocantidadM As Integer)
        ID = pID
        Cantidad = pCantidad
        Precio = pPrecio
        IdMoneda = pIdMoneda
    ...
    Public Function ConsultaReaderIVa(ByVal pIdVenta As Integer) As MySql.Data.MySqlClient.MySqlDataReader
        ...
        Return Comm.ExecuteReader
    End Function
    Public Sub SeparaKit(ByVal pIdVenta As Integer, ByVal pIdinventario As Integer, ByVal pCantidad As Double, ByVal pPrecio As Double, ByVal pIdMoneda As Integer, ByVal pDescripcion As String, ByVal pIdAlmacen As Integer, ByVal pIva As Double, ByVal pDescuento As Double, ByVal pIdVariante As Integer, ByVal pIdServicio As Integer, ByVal pIeps As Double, ByVal pivaretenido As Double, pCantidadM As Double, pTipoCantidadM As Integer)
        Dim STR As String
        Idinventario = pIdinventario
        Cantidad = pCantidad
        Precio = pPrecio
        ...
        STR += "insert into tblventasremisionesinventario(idremision,idinventario,cantidad,precio,descripcion,idmoneda,idalmacen,iva,extra,descuento,idvariante,idservicio,surtido,preciooriginal,ieps,ivaretenido,cantidadm,tipocantidadm) select " + IdRemision.ToString + ",tblinventariodetalles.idinventario,tblinventariodetalles.cantidad,(select precio from tblinventarioprecios where idinventario=tblinventariodetalles.idinventario and idlista=1 limit 1)*tblinventariodetalles.cantidad,(select nombre from tblinventario where idinventario=tblinventariodetalles.idinventario)," + IdMoneda.ToString + "," + IdAlmacen.ToString + ",(select iva from tblinventario where idinventario=tblinventariodetalles.idinventario),''," + Descuento.ToString + "," + IdVariante.ToString + "," + IdServicio.ToString + ",0,(select precio from tblinventarioprecios where idinventario=tblinventariodetalles.idinventario and idlista=1 limit 1),(select ieps from tblinventario where idinventario=tblinventariodetalles.idinventario),(select ivaretenido from tblinventario where idinventario=tblinventariodetalles.idinventario),tblinventariodetalles.cantidad,(select tipocontenido from tblinventario where idinventario=tblinventariodetalles.idinventario) from tblinventariodetalles inner join tblinventario on tblinventario.idinventario=tblinventariodetalles.idinventariop  where tblinventariodetalles.idinventariop=" + pIdinventario.ToString + ";"

Public Class dbComprasRemisiones
    ...
    Public Function ChecaFolioRepetidoi(ByVal pSerie As String, ByVal pFolio As Integer, ByVal pIdSucursal As Integer) As Boolean
        ...
        End If
    End Function
    Public Sub Guardar(ByVal pIdProveedor As Integer, ByVal pFecha As String, ByVal pFolio As String, ByVal pidMoneda As Integer, ByVal pIva As Double, ByVal pidSucursal As Integer, ByVal pSerie As String, ByVal pFolioi As Integer, ByVal pTipodeCambio As Double)
    ...
    Public Sub Usar(ByVal pID As Integer, ByVal pIdComprar As Integer)
        Comm.CommandText = "update tblcomprasremisiones set usado=1,idcomprar=" + pIdComprar.ToString + " where idremision=" + pID.ToString
        Comm.ExecuteNonQuery()
    End Sub
    ...
    Public Function Consulta(ByVal pFecha As String, ByVal pFecha2 As String, ByVal pNombreClave As String, ByVal pEstado As Byte, ByVal pFolio As String, ByVal pUsado As Byte, ByVal pIdSucursal As Integer) As DataView
        ...
        End If
        Comm.CommandText += " order by tblcomprasremisiones.fecha,folio"
    ...
    Public Sub ModificaInventario(ByVal pId As Integer)
        ...
        Comm.CommandText += "select spmodificainventariolotesf(tblcomprasremisioneslotes.idlote,tblcomprasremisioneslotes.cantidad-tblcomprasremisioneslotes.surtido,0,0,1) from tblcomprasremisioneslotes inner join tblcomprasremisionesdetalles on tblcomprasremisioneslotes.iddetalle = tblcomprasremisionesdetalles.iddetalle where tblcomprasremisionesdetalles.idremision= " + pId.ToString + "; "
        Comm.CommandText += "select spmodificainventarioaduanaf(tblcomprasremisionesaduana.idaduana,tblcomprasremisionesaduana.cantidad-tblcomprasremisionesaduana.surtido,0,0,1) from tblcomprasremisionesaduana inner join tblcomprasremisionesdetalles on tblcomprasremisionesaduana.iddetalle = tblcomprasremisionesdetalles.iddetalle where tblcomprasremisionesdetalles.idremision= " + pId.ToString + "; "
    ...
    Public Sub RegresaInventario(ByVal pId As Integer)
        ...
        Comm.CommandText += "select spmodificainventariolotesf(tblcomprasremisioneslotes.idlote,tblcomprasremisioneslotes.surtido,0,1,1) from tblcomprasremisioneslotes inner join tblcomprasremisionesdetalles on tblcomprasremisioneslotes.iddetalle=tblcomprasremisionesdetalles.iddetalle where tblcomprasremisionesdetalles.idremision=" + pId.ToString + ";"
        Comm.CommandText += "select spmodificainventarioaduanaf(tblcomprasremisionesaduana.idaduana,tblcomprasremisionesaduana.surtido,0,1,1) from tblcomprasremisionesaduana inner join tblcomprasremisionesdetalles on tblcomprasremisionesaduana.iddetalle=tblcomprasremisionesdetalles.iddetalle where tblcomprasremisionesdetalles.idremision=" + pId.ToString + ";"
        Comm.ExecuteNonQuery()
    ...
    Public Function ReporteComprasArticulos(ByVal pFecha1 As String, ByVal pFecha2 As String, ByVal pIdSucursal As Integer, ByVal pIdProveedor As Integer, ByVal pIdInventario As Integer, ByVal pidClasificacion As Integer, ByVal pidClasificacion2 As Integer, ByVal pidClasificacion3 As Integer, ByVal pidMoneda As Integer, ByVal pMostrarEnPesos As Byte) As DataView
        ...
        If pMostrarEnPesos = 0 Then
            ...
            "if(cd.idmoneda=2,cd.iva*cd.precio/100,cd.precio*c.tipodecambio*cd.iva/100) as ivacalc,if(cd.idmoneda=2,cd.ieps*cd.precio/100,cd.precio*c.tipodecambio*cd.ieps/100) as iepscalc,if(cd.idmoneda=2,cd.ivaretenido*cd.precio/100,cd.precio*c.tipodecambio*cd.ivaretenido/100) as ivaretcalc " + _
            "from tblcomprasremisiones c inner join tblcomprasremisionesdetalles cd on c.idremision=cd.idremision inner join tblinventario i on cd.idinventario=i.idinventario inner join tblproveedores p on c.idproveedor=p.idproveedor where c.estado=3 and c.fecha>='" + pFecha1 + "' and c.fecha<='" + pFecha2 + "'"
        Else
            ...
            "cd.iva*cd.precio/100 as ivacalc,cd.precio*cd.ieps/100 as iepscalc,cd.ivaretenido*cd.precio/100 as ivaretcalc " + _
            "from tblcomprasremisiones c inner join tblcomprasremisionesdetalles cd on c.idremision=cd.idremision inner join tblinventario i on cd.idinventario=i.idinventario inner join tblproveedores p on c.idproveedor=p.idproveedor where c.estado=3 and c.fecha>='" + pFecha1 + "' and c.fecha<='" + pFecha2 + "'"
        End If
    ...
    Public Function Reporte(ByVal pFecha1 As String, ByVal pFecha2 As String, ByVal pIdSucursal As Integer, ByVal pIdCliente As Integer, ByVal pMostrarEnPesos As Byte, ByVal pSoloCanceladas As Boolean) As DataView
        Dim DS As New DataSet
        If pMostrarEnPesos = 0 Then
            Comm.CommandText = "select v.idremision idventa,v.fecha,v.folioi as folio,concat(v.folio,' ',v.serie) as serie,v.estado,if(v.idmoneda=2,v.total,v.total*v.tipodecambio) as total,if(v.idmoneda=2,v.totalapagar,v.totalapagar*v.tipodecambio) as totalapagar,v.fecha,v.tipodecambio,v.idmoneda,c.nombre as cnombre,0 usado " + _
            "from tblcomprasremisiones v inner join tblproveedores c on v.idproveedor=c.idproveedor where v.usado=0 and v.fecha>='" + pFecha1 + "' and v.fecha<='" + pFecha2 + "'"
        Else
            Comm.CommandText = "select v.idremision idventa,v.fecha,v.folioi as folio,concat(v.folio,' ',v.serie) as serie,v.estado,v.total as total,v.totalapagar as totalapagar,v.fecha,v.tipodecambio,v.idmoneda,c.nombre as cnombre,0 usado " + _
            "from tblcomprasremisiones v inner join tblproveedores c on v.idproveedor=c.idproveedor where v.usado=0 and v.fecha>='" + pFecha1 + "' and v.fecha<='" + pFecha2 + "'"
        End If

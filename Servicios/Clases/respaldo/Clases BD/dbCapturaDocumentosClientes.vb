Public Class dbCapturaDocumentosClientes
    ...
    Public Function Consulta(ByVal pidcliente As Integer, ByVal pFolio As String, ByVal pEstado As Byte) As DataView
        ...
        Comm.CommandText = "select dc.iddocumento,dc.fecha,if(dc.tiposaldo=0,dc.serie,dc.seriereferencia) as seriereferencia,if(dc.tiposaldo=0,folio,folioreferencia) as folioreferencia,dc.totalapagar,case dc.estado when 2 then 'Pendiente' when 3 then 'Guardado' when 4 then 'Cancelado' end  as sestado,case dc.tiposaldo when 0 then 'Saldo Inicial' when 1 then 'Documento' end as tiposaldostr from tbldocumentosclientes dc inner join tblclientes c on dc.idcliente=c.idcliente where true"

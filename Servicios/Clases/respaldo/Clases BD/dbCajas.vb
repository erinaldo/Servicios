Public Class dbCajas
    ...
    Public Sub New(ByVal pID As Integer, ByVal Conexion As MySql.Data.MySqlClient.MySqlConnection)
        ...
        DReader.Close()
    End Sub
    Public Sub Guardar(ByVal pNombre As String, ByVal pIdSucursal As Integer, ByVal pSerie As String, ByVal pSerieCot As String, ByVal pSeriePed As String)
        Nombre = pNombre
        IdSucursal = pIdSucursal
        Serie = pSerie
        SerieCot = pSerieCot
        SeriePed = pSeriePed
        Comm.CommandText = "insert into tblcajas(nombre,idsucursal,serie,deposito,seriecot,serieped,idUsuarioAlta,fechaAlta,horaAlta,idUsuarioCambio,fechaCambio,horaCambio) values('" + Replace(Nombre, "'", "''") + "'," + IdSucursal.ToString + ",'" + Replace(Serie, "'", "''") + "',0,'" + Replace(SerieCot, "'", "''") + "','" + Replace(SeriePed, "'", "''") + "'," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "'," + GlobalIdUsuario.ToString() + ",'" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + TimeOfDay.ToString("HH:mm:ss") + "')"
        Comm.ExecuteNonQuery()
    End Sub
    Public Sub Modificar(ByVal pID As Integer, ByVal pNombre As String, ByVal pSerie As String, ByVal pSerieCot As String, ByVal pSeriePed As String)
        ID = pID
        Nombre = pNombre
        Serie = pSerie
        SerieCot = pSerieCot
        SeriePed = pSeriePed
        Comm.CommandText = "update tblcajas set nombre='" + Replace(Nombre, "'", "''") + "',serie='" + Replace(Serie, "'", "''") + "',seriecot='" + Replace(SerieCot, "'", "''") + "',serieped='" + Replace(SeriePed, "'", "''") + "', idUsuarioCambio=" + GlobalIdUsuario.ToString() + ", fechaCambio='" + DateTime.Now.ToString("yyyy/MM/dd") + "', horaCambio=" + TimeOfDay.ToString("HH:mm:ss") + "' where idcaja=" + ID.ToString
        Comm.ExecuteNonQuery()
    End Sub

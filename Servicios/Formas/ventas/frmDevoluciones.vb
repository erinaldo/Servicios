Public Class frmDevoluciones
    ...
    Private Sub Nuevo()
        ...
        EsElectrocnica = Sf.EsElectronica
    ...
    Private Sub Guardar()
        Try
            ...
            If idCliente <> 0 Then
                ...
                C.DaTotal(idDevolucion, IDsMonedas2.Valor(ComboBox2.SelectedIndex))
                C.Guardar(idCliente, Format(DateTimePicker1.Value, "yyyy/MM/dd"), CInt(TextBox2.Text), Desglozar, 0, TextBox11.Text, Sf.NoAprobacion, Sc.NoSerie, Sf.YearAprobacion, 0, IdsSucursales.Valor(ComboBox3.SelectedIndex), IdForma, TipoDeCambio, IDsMonedas2.Valor(ComboBox2.SelectedIndex), Isr, IvaRetenido, IdVenta, IdRemision, "")
                idDevolucion = C.ID
    ...
    Private Sub CadenaOriginal()
        ...
        Impresora = SA.DaImpresoraActiva(IdsSucursales.Valor(ComboBox3.SelectedIndex), GlobalIdEmpresa, True, 0, TiposDocumentos.VentaDevolucion)
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        'Dim SF As New dbSucursalesFolios(MySqlcon)
        'SF.BuscaFolios(V.IdSucursal, dbSucursalesFolios.TipoDocumentos.Devolucionn, 1)
        'e.Graphics.DrawString("DEVOLUCIÓN", Fuente, Brushes.Black, 155, 4)
        ...
        '    Y = Y + InsertaEnters(CadenaB, 60, Y, 4)
        ...
        'e.Graphics.DrawString(Format(V.TotalaPagar, "$#,###,##0.00").PadLeft(13), Fuente, Brushes.Black, 178, Y)
        ...
        '    CadenaB = f.PASELETRAS(System.Math.Round(V.TotalaPagar, 2), V.IdConversion)
        'Else
        '    CadenaB = f.PASELETRAS(System.Math.Round(V.TotalaPagar, 2), V.IdConversion)
        'End If

        'YB = Y
        'Y = Y + InsertaEnters(CadenaB, 63, Y, 4)
        ...
        'YB = Y
        'CadenaB = Cadena
        'Y = Y + InsertaEnters(CadenaB, 105, Y, 4)
        ...
        'SelloB = Sello
        'YB = Y
        'Y = Y + InsertaEnters(SelloB, 105, Y, 4)
    ...
    Private Sub CadenaOriginali(ByVal pEstado As Byte)
        ...
        If V.NoCertificadoSAT <> "Error" Then
            ...
            'Dim Bytes() As Byte = Encoder.GetBytes(pCadenaOriginal)
            'IO.Directory.CreateDirectory(My.Settings.rutapdf + "\" + Format(CDate(V.Fecha), "yyyy") + "\")
            'IO.Directory.CreateDirectory(My.Settings.rutapdf + "\" + Format(CDate(V.Fecha), "yyyy") + "\" + Format(CDate(V.Fecha), "MM") + "\")
            ...
            'en.GuardaArchivo(My.Settings.rutaxml + "\" + Format(CDate(V.Fecha), "yyyy") + "\" + Format(CDate(V.Fecha), "MM") + "\XMLFacCFDI-" + V.Serie + V.Folio.ToString + ".xml", Bytes)
            'en.GuardaArchivoTexto(My.Settings.rutaxmldev + "\" + Format(CDate(V.Fecha), "yyyy") + "\" + Format(CDate(V.Fecha), "MM") + "\XMLDEVCFDI" + V.Serie + V.Folio.ToString + ".xml", strXML, System.Text.Encoding.UTF8)
            ...
            'If My.Settings.impresoraPDF = "Bullzip PDF Printer" Then Bullzip.PdfWriter.PdfUtil.WaitForFile(RutaPDF + "\PDFDEVCFDI" + V.Serie + V.Folio.ToString + ".pdf", 1000)
            'Dim op As New dbOpciones(MySqlcon)
    ...
    Private Sub LlenaNodosImpresion()
        ...
        Else
            ImpND.Add(New NodoImpresionN("", "docafecta", V.DaDatosDocumento(idDevolucion), 0), "docafecta")
        End If
        If V.Cliente.DireccionFiscal = 0 Then
        ...
        CodigoBidimensional = CB.Encode("?re=" + Sucursal.RFC + "&rr=" + V.Cliente.RFC + "&tt=" + Format(V.TotalVenta, "0000000000.000000") + "&id=" + V.uuid, System.Text.Encoding.Default)
        NumeroPagina = 1
    End Sub
    ...
    '    Try
    '        e.DrawImage(Image.FromFile(SA.DaRuta(TiposDocumentos.VentaDevolucion, IdsSucursales.Valor(ComboBox3.SelectedIndex), GlobalIdEmpresa, True)), 1, 1, CInt(864 / 40 * 10), CInt(1116 / 40 * 10))
    '    Catch ex As Exception
    ...
    Private Sub DibujaPaginaN(ByRef e As System.Drawing.Graphics)
        ...
        If TipoImpresora = 0 Then
            ImpDb.DaZonaDetalles(TiposDocumentos.VentaDevolucion, IdsSucursales.Valor(ComboBox3.SelectedIndex))
        Else
            ImpDb.DaZonaDetalles(TiposDocumentos.VentaDevolucion + 1000, IdsSucursales.Valor(ComboBox3.SelectedIndex))
        End If
        If TipoImpresora = 0 Then
    ...
    Private Sub DibujaPaginaEstatico(ByRef e As System.Drawing.Graphics, ByVal pZonaY As Integer, ByVal pZonaYL As Integer, ByVal pZonaRG As Integer, ByVal pZonaAlt As Integer)
        ...
        Try
            ...
            If TipoImpresora = 0 Then
                e.DrawImage(Image.FromFile(SA.DaRuta(TiposDocumentos.VentaDevolucion, IdsSucursales.Valor(ComboBox3.SelectedIndex), GlobalIdEmpresa, True)), 1, 1, CInt(864 / 40 * 10), CInt(1116 / 40 * 10))
            Else
                e.DrawImage(Image.FromFile(SA.DaRuta(TiposDocumentos.VentaDevolucion + 1000, IdsSucursales.Valor(ComboBox3.SelectedIndex), GlobalIdEmpresa, True)), 1, 1, CInt(864 / 40 * 10), CInt(1116 / 40 * 10))
            End If
            'Else
            'If TipoImpresora = 0 Then
    ...
    Private Sub Imprimir()
        ...
        Dim RutaPDF As String
        Dim Archivos As New dbSucursalesArchivos
        RutaPDF = Archivos.DaRutaArchivos(IdsSucursales.Valor(ComboBox3.SelectedIndex), GlobalIdEmpresa, dbSucursalesArchivos.TipoRutas.DevolucionesPDF, True)
        ...
        PrintDocument1.DocumentName = "Devolucion" + TextBox11.Text + TextBox2.Text
        ...
        LlenaNodosImpresion()
        If TipoImpresora = 0 Then
            LlenaNodos(IdsSucursales.Valor(ComboBox3.SelectedIndex), TiposDocumentos.VentaDevolucion)
        Else
            LlenaNodos(IdsSucursales.Valor(ComboBox3.SelectedIndex), TiposDocumentos.VentaDevolucion + 1000)
        End If
        PrintDocument1.Print()
    End Sub

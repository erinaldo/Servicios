Public Class frmNotasdeCreditoN
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        'e.Graphics.DrawString("NOTA DE CRÉDITO", Fuente, Brushes.Black, 155, 4)
        ...
        'e.Graphics.DrawLine(Pluma, 5, 60, 210, 60)
        ...
        '    Else
        '        CadenaB = DR("descripcion") + " | Nota de Cargo: " + DR("serieventa") + DR("folioventa").ToString
        '    End If
        '    Y = InsertaEnters(CadenaB, 60, Y, 4)
        '    Rec = New RectangleF(5, YB, 160, 200)
        '    If DR("idinventario") = 0 Then
        '        e.Graphics.DrawString(DR("descripcion") + " | Factura: " + DR("serieventa") + DR("folioventa").ToString, Fuente, Brushes.Black, Rec, strF)
        '    Else
        '        e.Graphics.DrawString(DR("descripcion") + " | Nota de Cargo: " + DR("serieventa") + DR("folioventa").ToString, Fuente, Brushes.Black, Rec, strF)
        '    End If
    ...
    '    PrintDocument1.DocumentName = "NC-" + V.Serie + V.Folio.ToString
    ...
    '    Bullzip.PdfWriter.PdfUtil.WaitForFile(RutaPDF + "\NC-" + V.Serie + V.Folio.ToString + ".pdf", 1000)
    ...
    Private Sub CadenaOriginal()
        ...
        Impresora = SA.DaImpresoraActiva(IdsSucursales.Valor(ComboBox3.SelectedIndex), GlobalIdEmpresa, True, 0, TiposDocumentos.VentaNotadeCredito)
    ...
    Private Sub LlenaNodosImpresion()
        ...
        If Sucursal.HayExp Then
            ...
            Else
                ImpND.Add(New NodoImpresionN("", "lugar", Sucursal.LugarExp + " " + Replace(V.FechaTimbrado, "T", " "), 0), "lugar")
            End If
        Else
            If Sucursal.Ciudad2 <> "" And Sucursal.Estado2 <> "" Then
        ...
        While DR.Read
            ...
            ImpNDD.Add(New NodoImpresionN("", "preciou", Format((DR("precio") / (1 + ((DR("iva") - V.ISR - V.IvaRetenido) / 100))) / DR("cantidad"), O._formatoPrecioU).PadLeft(O.EspacioPrecioUnitacio), 0), "preciou" + Format(Cont, "000"))
            ImpNDD.Add(New NodoImpresionN("", "importe", Format((DR("precio") / (1 + ((DR("iva") - V.ISR - V.IvaRetenido) / 100))), O._formatoImporte).PadLeft(O.EspacioImporte), 0), "importe" + Format(Cont, "000"))
            CuantosRenglones += 1
            Cont += 1
        End While
        DR.Close()
        ...
        ImpND.Add(New NodoImpresionN("", "comentario", V.ComentarioNC, 0), "comentario")
        ImpND.Add(New NodoImpresionN("", "subtotal", Format(V.Subtotal - V.TotalIva + V.TotalIvaRetenida + V.TotalISR, O._formatoSubtotal).PadLeft(O.EspacioSubtotal), 0), "subtotal")
        ...
        End If
        If V.IvaRetenido <> 0 Then
            ImpNDDi.Add(New NodoImpresionN("IVARet. " + Format(V.IvaRetenido, "#0.00") + "%:", "IVARet. " + Format(V.IvaRetenido, "#0.00") + "%:", Format(V.TotalIvaRetenida, O._formatoIva).PadLeft(O.EspacioIva), 0), "iva" + Format(Cont, "00"))
            ...
            Cont += 1
        Else
            'ImpND.Add(New NodoImpresionN("IVARet. " + Format(V.IvaRetenido, "#0.00") + "%:", "IVAret. " + Format(V.Iva, "#0.00") + "%:", Format(V.TotalIvaRetenida, O._formatoIva).PadLeft(O.EspacioIva), 0), "ivaret")
        End If
    ...
    '    Try
    '        e.DrawImage(Image.FromFile(SA.DaRuta(TiposDocumentos.VentaNotadeCredito, IdsSucursales.Valor(ComboBox3.SelectedIndex), GlobalIdEmpresa, True)), 1, 1, CInt(864 / 40 * 10), CInt(1116 / 40 * 10))
    '    Catch ex As Exception
    ...
    '            YExtra = 0
    '            If YCoord >= 146 And Posicion > 0 Then
    '                MasPáginas = True
    '                Exit While
    '            End If
    '            For Each n As NodoImpresionN In ImpNDi
    ...
    Private Sub DibujaPaginaN(ByRef e As System.Drawing.Graphics)
        ...
        If TipoImpresora = 0 Then
            ImpDb.DaZonaDetalles(TiposDocumentos.VentaNotadeCredito, IdsSucursales.Valor(ComboBox3.SelectedIndex))
        Else
            ImpDb.DaZonaDetalles(TiposDocumentos.VentaNotadeCredito + 1000, IdsSucursales.Valor(ComboBox3.SelectedIndex))
        End If
        If TipoImpresora = 0 Then
    ...
    Private Sub DibujaPaginaEstatico(ByRef e As System.Drawing.Graphics, ByVal pZonaY As Integer, ByVal pZonaYL As Integer, ByVal pZonaRG As Integer, ByVal pZonaAlt As Integer)
        ...
        Try
            ...
            If TipoImpresora = 0 Then
                e.DrawImage(Image.FromFile(SA.DaRuta(TiposDocumentos.VentaNotadeCredito, IdsSucursales.Valor(ComboBox3.SelectedIndex), GlobalIdEmpresa, True)), 1, 1, CInt(864 / 40 * 10), CInt(1116 / 40 * 10))
            Else
                e.DrawImage(Image.FromFile(SA.DaRuta(TiposDocumentos.VentaNotadeCredito + 1000, IdsSucursales.Valor(ComboBox3.SelectedIndex), GlobalIdEmpresa, True)), 1, 1, CInt(864 / 40 * 10), CInt(1116 / 40 * 10))
            End If
            'Else
            'If TipoImpresora = 0 Then
    ...
    'Private Sub Imprimir(ByVal SerieFolio As String)

    '    Dim RutaPDF As String
    '    Dim Archivos As New dbSucursalesArchivos
    '    RutaPDF = Archivos.DaRutaArchivos(IdsSucursales.Valor(ComboBox3.SelectedIndex), GlobalIdEmpresa, dbSucursalesArchivos.TipoRutas.NotasdeCreditoPDF, True)
    '    IO.Directory.CreateDirectory(RutaPDF + "\" + Format(dtpFechaPago.Value, "yyyy") + "\")
    '    IO.Directory.CreateDirectory(RutaPDF + "\" + Format(dtpFechaPago.Value, "yyyy") + "\" + Format(dtpFechaPago.Value, "MM") + "\")
    '    RutaPDF = RutaPDF + "\" + Format(dtpFechaPago.Value, "yyyy") + "\" + Format(dtpFechaPago.Value, "MM")



    '    PrintDocument1.DocumentName = "PDFNCR" + SerieFolio
    '    If My.Settings.impresoraPDF = "Bullzip PDF Printer" Then
    '        Dim obj As New Bullzip.PdfWriter.PdfSettings
    '        obj.Init()
    '        obj.PrinterName = My.Settings.impresoraPDF
    ...
    '    PrintDocument1.PrinterSettings.PrinterName = My.Settings.impresoraPDF
    '    LlenaNodosImpresion()
    '    LlenaNodos(IdsSucursales.Valor(ComboBox3.SelectedIndex), TiposDocumentos.VentaNotadeCredito)
    '    PrintDocument1.Print()
    'End Sub
    Private Sub Imprimir(ByVal SerieFolio As String)
        ...
        End If
        'RutaPDF = RutaPDF + "\" + Format(dtpFechaPago.Value, "yyyy") + "\" + Format(dtpFechaPago.Value, "MM")
        PrintDocument1.DocumentName = "NCredito" + SerieFolio
        Dim SA As New dbSucursalesArchivos
        Dim Impresora As String
        Impresora = SA.DaImpresoraActiva(IdsSucursales.Valor(ComboBox3.SelectedIndex), GlobalIdEmpresa, True, 0, TiposDocumentos.VentaNotadeCredito)
        TipoImpresora = SA.TipoImpresora
        ...
        If TipoImpresora = 0 Then
            LlenaNodos(IdsSucursales.Valor(ComboBox3.SelectedIndex), TiposDocumentos.VentaNotadeCredito)
        Else
            LlenaNodos(IdsSucursales.Valor(ComboBox3.SelectedIndex), TiposDocumentos.VentaNotadeCredito + 1000)
        End If
        PrintDocument1.Print()
    End Sub
    ...
    Private Sub txtdatoscliente_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtdatoscliente.TextChanged

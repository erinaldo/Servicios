Public Class frmNotasdeCargoC
    ...
    Private Sub frmVentasN_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        If Estado = Estados.SinGuardar Then
            If MsgBox("Esta nota de cargo no ha sido guardada. ¿Cerrar la ventana de todas maneras? Los datos se perderán.", MsgBoxStyle.YesNo, GlobalNombreApp) = MsgBoxResult.Yes Then
                ...
                Dim C As New dbNotasdeCargoCompras(idNota, MySqlcon)
    ...
    Private Sub frmNotasdeCargoC_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub frmVentasN_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        LlenaCombos("tblconceptosnotascompras", cmbConcepto, "nombre", "nombret", "idconceptonotacompra", IdsTipos, " tipo=0 and idconceptonotacompra>0")
    ...
    Private Sub Nuevo()
        ...
        'Sf.BuscaFolios(IdsSucursales.Valor(ComboBox3.SelectedIndex), dbSucursalesFolios.TipoDocumentos.NotasDeCargo, 1)
        TextBox2.Text = ""
        Dim Sf As New dbSucursalesFolios(MySqlcon)
        Sf.BuscaFolios(IdsSucursales.Valor(ComboBox3.SelectedIndex), dbSucursalesFolios.TipoDocumentos.ComprasNotasCargo, 0)
    ...
    Private Sub Modificar(ByVal pEstado As Byte)
        Try
            ...
            Dim C As New dbNotasdeCargoCompras(MySqlcon)
            ...
            If GlobalPermisos.ChecaPermiso(PermisosN.Compras.NotasdeCargoAlta, PermisosN.Secciones.Compras) = False And pEstado = Estados.Guardada Then
                MensajeError += " No tiene permiso para realizar esta operación."
            End If
            If GlobalPermisos.ChecaPermiso(PermisosN.Compras.NotasdeCargoCancelacion, PermisosN.Secciones.Compras) = False And pEstado = Estados.Cancelada Then
                MensajeError += " No tiene permiso para realizar esta operación."
            End If
            'End If
            If MensajeError = "" Then
                ...
                C.Modificar(idNota, Format(DateTimePicker1.Value, "yyyy/MM/dd"), TextBox2.Text, 0, pEstado, C.Subtotal, C.TotalNota, idProveedor, CDbl(TextBox10.Text), IDsMonedas2.Valor(ComboBox2.SelectedIndex), IdsTipos.Valor(cmbConcepto.SelectedIndex), TextBox14.Text, CInt(TextBox15.Text), TextBox11.Text, TextBox16.Text)
    ...
    Private Sub Guardar()
        Try
            ...
            If GlobalPermisos.ChecaPermiso(PermisosN.Compras.NotasdeCargoAlta, PermisosN.Secciones.Compras) = True Then
                If idProveedor <> 0 Then
                    ...
                    If MensajeError = "" Then
                        ...
                        'Sf.BuscaFolios(IdsSucursales.Valor(ComboBox3.SelectedIndex), dbSucursalesFolios.TipoDocumentos.NotasDeCargo, 1)
                        'Dim Sc As New dbSucursalesCertificados(Sf.IdCertificado, MySqlcon)
                        C.Guardar(idProveedor, Format(DateTimePicker1.Value, "yyyy/MM/dd"), TextBox2.Text, 0, IdsSucursales.Valor(ComboBox3.SelectedIndex), CDbl(TextBox10.Text), IDsMonedas2.Valor(ComboBox2.SelectedIndex), IdsTipos.Valor(cmbConcepto.SelectedIndex), CInt(TextBox15.Text), TextBox11.Text, TextBox16.Text)
                        idNota = C.ID
                        Estado = 1
    ...
    '            If p.BuscaArticulo(TextBox3.Text) Then
    ...
    Private Sub LlenaDatosVenta()
        Dim C As New dbNotasdeCargoCompras(idNota, MySqlcon)
        ConsultaOn = False
        ComboBox3.SelectedIndex = C.IdSucursal
        ConsultaOn = True
    ...
    Private Sub ConsultaDetalles()
        Try
            ...
            Dim CD As New dbNotasdeCargoComprasDetalles(MySqlcon)
    ...
    Private Sub LlenaDatosDetallesA()
        Try
            ...
            Dim CD As New dbNotasdeCargoComprasDetalles(IdDetalle, MySqlcon)
    ...
    Private Sub BotonNuevo()
        If Estado = Estados.SinGuardar Then
            If MsgBox("Esta nota de cargo no ha sido guardada. ¿Desea iniciar una nota? Los datos actuales se perderán.", MsgBoxStyle.YesNo, GlobalNombreApp) = MsgBoxResult.Yes Then
                Dim c As New dbNotasdeCargoCompras(MySqlcon)
                c.Eliminar(idNota)
                Nuevo()
            End If
    ...
    '    Else
    '        MsgBox("Se necesita que un producto tenga al menos una variante.", MsgBoxStyle.Critical, NombreApp)
    '        IdVariante = 0
    '    End If
    ...
    Private Sub Button10_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button10.Click
        Dim f As New frmNotasdeCargoComprasConsulta(ModosDeBusqueda.Secundario, "")
        f.ShowDialog()
    ...
    Private Sub CadenaOriginal()
        ...
        Dim V As New dbNotasdeCargoCompras(idNota, MySqlcon)
        ...
        PrintDocument1.DocumentName = "NotasCargo - " + V.Folio.ToString() + " - " + V.Proveedor.Nombre.ToString()  'Modificado ESTO SE NECESITA PONER AFUERA
        PrintDocument1.PrinterSettings.PrinterName = Rep.Imprimir("NotasCargo", "NotasCargo - " + V.Folio.ToString() + " - " + V.Proveedor.Nombre.ToString(), Date.Today.ToString("dd-MM-yyyy"))
        ...
        End If
        Rep.Posicion = 0
        Rep.LlenaNodosImpresionNotasCargo(Format(V.Folio, "0000"), Replace(V.Fecha, "/", "-") + " " + V.Hora, Format(V.Subtotal, "$#,###,##0.00"), Format(V.TotalIva, "$#,###,##0.00"), Format(V.TotalaPagar, "$#,###,##0.00"), "00", CadenaB, " ", Sucursal.NombreFiscal, Sucursal.Direccion + " " + Sucursal.NoExterior, Sucursal.Colonia + ", " + Sucursal.Ciudad, Sucursal.CP, Sucursal.RFC, Sucursal.Telefono, Sucursal.Telefono, Sucursal.Email, V.Proveedor.Nombre, V.Proveedor.Direccion, V.Proveedor.Colonia, V.Proveedor.CP, V.Proveedor.Ciudad, V.Proveedor.RFC, Tabla, idNota, V.Proveedor.NoInterior, V.Proveedor.NoExterior, V.Proveedor.Estado, V.Proveedor.Pais, V.Comentario)
        ...
        'PrintDocument1.DocumentName = "PDFNCAC-" + V.Folio.ToString
        ...
        'PrintDocument1.Print()
        ''Bullzip.PdfWriter.PdfUtil.WaitForFile(My.Settings.rutapdfnca + "\" + Format(CDate(V.Fecha), "yyyy") + "\" + Format(CDate(V.Fecha), "MM") + "\PDFNCAC-" + V.Folio.ToString + ".pdf", 1000)
        ...
        ''                Dim O As New dbOpciones(MySqlcon)
        ''                Dim C As String
        ''                C = "Eviado por: " + O._NombreEmpresa + vbNewLine + "RFC: " + O._RFC + vbNewLine + "NOTA DE CREDITO" + vbNewLine + "Folio: " + V.Serie + V.Folio.ToString + vbNewLine + "Comprobante fiscal digital enviado por medio del sistema de facturación de Pull System Soft, S.A. de C.V." + vbNewLine + "http://pullsystemsoft.com" + vbNewLine + "Este es un mensaje enviado automáticamente, no responda a este mensaje a la dirección de la que se envió."
        ''                M.send("Comprobante fiscal digital Nota de crédito " + V.Serie + V.Folio.ToString, C, V.Cliente.Email, V.Cliente.Nombre, My.Settings.rutapdfnca + "\" + Format(CDate(V.Fecha), "yyyy") + "\" + Format(CDate(V.Fecha), "MM") + "\PDFNCA" + V.Serie + V.Folio.ToString + ".pdf", My.Settings.rutaxmlnca + "\" + Format(CDate(V.Fecha), "yyyy") + "\" + Format(CDate(V.Fecha), "MM") + "\XMLNCA-" + V.Serie + V.Folio.ToString + ".xml")
        ''            End If
        ''        End If
        ''    Catch ex As Exception
        ''        MsgBox("No se puedo enviar el correo, verifique la configuración de correo o el correo del cliente." + vbCrLf + ex.Message, MsgBoxStyle.Critical, NombreApp)
        ''    End Try
        ''End If
    End Sub
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        'e.Graphics.DrawString("Nombre: " + V.Proveedor.Nombre, FuenteArialC, Brushes.Black, 5, 34)
        ...
        'e.Graphics.DrawString("NOTA DE CARGO - Compras", Fuente, Brushes.Black, 155, 4)
    ...
    Private Sub GeneraPoliza()
        Try
            ...
            If Op.IntegrarContabilidad = 1 And GlobalconIntegracion Then
                ...
                '    credito = 1
                'End If
                cuantas = M.CuantasHay(9, Canceladas, credito)
                If cuantas > 0 Then
                    If cuantas = 1 Then
                        M.ID = M.DaMascaraActiva(9, Canceladas, credito)
                    Else
                        Dim f As New frmContabilidadMascarasConsulta(ModosDeBusqueda.Secundario, False, 9)
                        f.ShowDialog()
                        If f.DialogResult = Windows.Forms.DialogResult.OK Then
                            M.ID = f.IdMascara
                        Else

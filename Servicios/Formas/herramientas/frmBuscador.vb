Public Class frmBuscador
    ...
    Public EquipoSucursal As dbClientesEquipos
    ...
    Public Enum TipoDeBusqueda As Integer
        ...
        Trabajador = 8
        ProveedorDiot = 9
        EquiposSucursales = 10
        ArticuloNoInv = 11
        SoloRestaurante = 12
    End Enum
    Private Sub Consulta()
        If TipoB = TipoDeBusqueda.Articulo Then
            ConsultaInventario(0, 1, 0)
        End If
        If TipoB = TipoDeBusqueda.Proveedor Then
            ConsultaProveedores(0)
        End If
        If TipoB = TipoDeBusqueda.ProveedorDiot Then
            ConsultaProveedores(1)
        End If
        ...
        If TipoB = TipoDeBusqueda.ArticuloProducto Then
            ...
            ConsultaInventario(0, 0, 0)
        End If
        If TipoB = TipoDeBusqueda.ArticuloProductoInv Then
            'ConsultaInventarioProductos(1)
            ConsultaInventario(1, 0, 0)
        End If
        If TipoB = TipoDeBusqueda.ArticuloInv Then
            ConsultaInventario(1, 1, 0)
        End If
        If TipoB = TipoDeBusqueda.ArticuloNoInv Then
            ConsultaInventario(2, 1, 0)
        End If
        If TipoB = TipoDeBusqueda.Trabajador Then
            ConsultaTrabajadores()
        End If
        If TipoB = TipoDeBusqueda.EquiposSucursales Then
            ConsultaEquiposSucursales()
        End If
        If TipoB = TipoDeBusqueda.SoloRestaurante Then
            ConsultaInventario(0, 1, 1)
        End If
    ...
    End Sub
    Private Sub ConsultaProveedores(ByVal Diot As Byte)
        Try
            If Diot > 0 Then Diot += 1
            Dim P As New dbproveedores(MySqlcon)
            DGConsulta.DataSource = P.Consulta(TextBox1.Text, Diot)
            ...
            DGConsulta.Columns(3).Width = 150
    ...
    Private Sub ConsultaClientes()
        ...
        End Try
    End Sub
    Private Sub ConsultaEquiposSucursales()
        Try
            Dim P As New dbClientesEquipos(MySqlcon)
            DGConsulta.DataSource = P.ConsultaSucLibres(idAlmacen, TextBox1.Text)
            DGConsulta.Columns(0).Visible = False
            DGConsulta.Columns(3).Visible = False
            DGConsulta.Columns(1).HeaderText = "Descripción"
            DGConsulta.Columns(2).HeaderText = "Matrícula"
            'DGConsulta.Columns(3).HeaderText = "RFC"
            DGConsulta.Columns(2).Width = 150
    ...
    Private Sub ConsultaInventario(ByVal pInv As Byte, ByVal peskit As Byte, pSoloRest As Byte)
        Try

            Dim I As New dbInventario(MySqlcon)
            DGConsulta.DataSource = I.Consulta(idAlmacen, TextBox1.Text, , , True, , , , pInv, GlobalpFabricante, GlobalModoBusqueda, peskit, 1, pSoloRest, SV, SC, SI)
            DGConsulta.Columns(0).Visible = False
            DGConsulta.Columns(1).HeaderText = "Código"
    ...
    Private Sub RegresaValor()
        Try
            If DGConsulta.RowCount > 0 Then
                ...
                If TipoB = TipoDeBusqueda.Articulo Or TipoB = TipoDeBusqueda.ArticuloInv Or TipoB = TipoDeBusqueda.ArticuloNoInv Or TipoB = TipoDeBusqueda.SoloRestaurante Then
                    Inventario = New dbInventario(ID, MySqlcon)
                    Tipo = "I"
                End If
                If TipoB = TipoDeBusqueda.Proveedor Or TipoB = TipoDeBusqueda.ProveedorDiot Then
                ...
                End If
                If TipoB = TipoDeBusqueda.EquiposSucursales Then
                    EquipoSucursal = New dbClientesEquipos(ID, MySqlcon, 1)
                    Tipo = "EQ"
                End If

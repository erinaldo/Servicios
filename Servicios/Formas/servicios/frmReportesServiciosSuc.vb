Public Class frmReportesServiciosSuc
    ...
    Private Sub frmReportesServiciosSuc_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Me.Icon = GlobalIcono
    ...
    Private Sub btnBuscarCliente_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'Dim B As New frmBuscador(frmBuscador.TipoDeBusqueda.Sucursal, 0)
        'B.ShowDialog()
        'If B.DialogResult = Windows.Forms.DialogResult.OK Then

        '    txtCliente.Text = B.Sucursal.Nombre
        '    idCliente = B.Sucursal.ID
    ...
    Private Sub cargarEquipos()
        If idCliente <> -1 Then
            ...
            Dt = P.buscarEquiposSuc(idCliente)
    ...
    Private Sub btnImprimir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnImprimir.Click
        If tipo = "Servicios" Then
            ...
            Rep.SetParameterValue("tipo", "SUCURSAL")
            ...
            RV.Show()
        End If
        If tipo = "Sucursales - Equipos" Then
            ...
            If idCliente = -1 Then
                ...
                Rep.SetParameterValue("dato", "SUCURSAL")
                Dim RV As New frmReportes(Rep, False)
                RV.Show()
            ...
            Else
                ...
                If NoErrores = True Then
                    ...
                    Dim nombreEmpresa As String = q.nombreClientesuc(idCliente)
                    Dim rfc As String = q.ConsultaRFCsuc(idCliente)
                    Dim direccion As String = q.Consultadireccionsuc(idCliente)
                    Dim direccion2 As String = q.direccion2suc(idCliente)
        ...
        If tipo = "Componentes de equipo" Then
            ...
            Else
                ...
                Dim q As New dbServicios(MySqlcon)
                'If q.Existesuc(txtCliente.Text) = 0 Then
                '    NoErrores = False
                '    MensajeError += vbCrLf + "Debe seleccionar un cliente registrado"
                '    txtCliente.BackColor = Color.FromArgb(250, 150, 150)
                'Else
                '    idCliente = q.obtIDsuc(txtCliente.Text)
                'End If


                If NoErrores = True Then
                    P.equipoDatossuc(idEquipo)
                    ...
                    Rep.SetParameterValue("DATOS", "SUCURSAL")
    ...
    Private Function consumibles() As DataSet
        ...
        For i As Integer = 0 To Dt.Rows.Count() - 1
            ...
            If P.cuantosConsumiblessuc(Dt.Rows(i)(1), Dt.Rows(i)(3)) > 0 Then
                tablaConsumibles = P.consumiblessuc(Dt.Rows(i)(1), Dt.Rows(i)(3)) 'asegurarse que se rdenen de mayor a menor la fehca
                Dim dr1 As DataRow
    ...
    Private Function servicios() As DataSet
        ...
        ' TablaFull.Columns.Add("Tiempo vida")
        Dt2 = P.buscarServiciosSuc(conversionFecha(dtpFecha.Value), conversionFecha(dtpFecha2.Value))

        For i As Integer = 0 To Dt2.Rows.Count() - 1
            Dim dr1 As DataRow
            dr1 = TablaFull.NewRow()
            dr1("idServicio") = Dt2.Rows(i)(0).ToString
            dr1("Equipo") = P.ConsultanombreEquiposuc(Integer.Parse(Dt2.Rows(i)(11).ToString))
            ...
            dr1("Estado") = P.ConsultanombreESTADO(Dt2.Rows(i)(6).ToString)
            dr1("cliente") = P.ConsultaSucursal(Integer.Parse(Dt2.Rows(i)(7).ToString))
            TablaFull.Rows.Add(dr1)
            'P.buscarArticulo(tablaConsumibles.Rows(0)(2).ToString)
            ' diasRestantes = DateDiff("d", conversionFecha(Date.Now()), conversionFecha(diaFinal))
        Next
    ...
    Private Function clienteEquipos() As DataSet
        ...
        Dt2 = P.buscarEquioposClientesuc(idCliente)

        For i As Integer = 0 To Dt2.Rows.Count() - 1

            Dt3 = q.detallesEquiposuc(Integer.Parse(Dt2.Rows(i)(0)))
            ...
            Else
                ...
                For j As Integer = 0 To Dt3.Rows.Count() - 1
                    ...
                    If q.cuantosConsumiblessuc(Dt3.Rows(j)(1), Dt3.Rows(j)(3)) > 0 Then
                        tablaConsumibles = q.consumiblessuc(Dt3.Rows(j)(1), Dt3.Rows(j)(3)) 'asegurarse que se rdenen de mayor a menor la fehca
                        hoy = DateTime.ParseExact(tablaConsumibles.Rows(0)(0).ToString, "yyyy-MM-dd", Nothing)
    ...
    Private Function clienteEquiposTodos() As DataSet
        ...
        For i As Integer = 0 To Dt2.Rows.Count() - 1
            ...
            If Dt3.Rows.Count = 0 Then
                ...
                dr1("Cliente") = P.nombreClientesuc(idCliente)
                dr1("RFC") = P.ConsultaRFCsuc(idCliente)
                dr1("Direccion") = P.Consultadireccionsuc(idCliente)
                dr1("Direccion2") = P.direccion2suc(idCliente)
                dr1("idEquipo") = Dt2.Rows(i)(0).ToString
                dr1("Nombre") = Dt2.Rows(i)(1).ToString
                dr1("Marca") = Dt2.Rows(i)(2).ToString
                dr1("Modelo") = Dt2.Rows(i)(3).ToString
    ...
    Private Function HistorialEspecifico() As DataSet
        ...
        Try
            ...
            Dim estado As String
            TablanumServicio = P.ConsultaServiciosIdSuc(idEquipo).ToTable
            ...
            For i As Integer = 0 To TablanumServicio.Rows.Count() - 1
                ...
                TablaServicios = SE.ConsultaTodosServiciossuc(idEquipo, pidServicio).ToTable
                ...
                For j As Integer = 0 To TablaServicios.Rows.Count() - 1
                    ...
                    P.detallesServiciosuc(pidServicio)
                    'detalles servicio
    ...
    Private Function HistorialTodos() As DataSet
        ...
        Dim pidServicio As Integer
        Dim estado As String
        tablaEquipos = P.idEquipoConServiciosuc().ToTable()
        'datos del equipo 
        TablaFull.Columns.Add("idEquipo")
        TablaFull.Columns.Add("nombre")
        TablaFull.Columns.Add("marca")
    ...
    Private Function HistoriaTecnico() As DataSet
        ...
        'Dim pidServicio As Integer
        'Dim estado As String
        Tabla = P.buscarServiciosTecnicosuc(idTecnico, idClasificacion, idSubClasificacion).ToTable()
        ...
        For i As Integer = 0 To Tabla.Rows.Count() - 1
            ...
            dr1("Equipo") = P.buscarEquipoNombreSUC(Tabla.Rows(i)(7).ToString)
    ...
    Private Sub cmbSucursal_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbSucursal.SelectedIndexChanged
        If cmbSucursal.SelectedIndex <> 0 Then
            idCliente = IdsSucursales.Valor(cmbSucursal.SelectedIndex)
        Else
            idCliente = -1
           
        End If
        If lstTipos.SelectedIndex = 2 Or lstTipos.SelectedIndex = 3 Then
            cargarEquipos()
        End If
    End Sub
End Class
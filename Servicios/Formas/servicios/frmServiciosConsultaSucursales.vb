Public Class frmServiciosConsultaSucursales
    Dim ConsultaOn As Boolean = True
    Dim IdsEstado As New elemento
    Private Sub frmServiciosConsultaSucursales_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ConsultaOn = False
        Try
            Me.Icon = GlobalIcono
        ...
        ConsultaOn = True
        'consultaTodos()
        If chkTiempoReal.Checked Then consultaEspecifica()
        TextBox1.Focus()
    End Sub
    Private Sub AbreDetalles()
        Dim F As New frmServiciosDetalles(DGServicios.Item(0, DGServicios.CurrentCell.RowIndex).Value, DGServicios.Item(7, DGServicios.CurrentCell.RowIndex).Value, 1)
        F.ShowDialog()
        F.Dispose()
    ...
    Public Sub consultaEspecifica()
        Try
            If ConsultaOn Then
                ...
                End If
                Tabla = S.ConsultaEspecificaSucursales(Format(DateTimePicker2.Value, "yyyy/MM/dd"), Format(DateTimePicker3.Value, "yyyy/MM/dd"), TextBox1.Text, x, y).ToTable
                For i As Integer = 0 To Tabla.Rows.Count() - 1
                    Dim dr As DataRow
